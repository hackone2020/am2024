...
<body>
 <div id="tit" class="tit" >
      ...
      <div class="tit_center floatleft font_bold"><span class="font_g">注单管理&nbsp;</span><span class="font_b">流水注单&nbsp;</span>
      </div>
   ...
   <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        <tbody>
            <tr>
            <td width="10%" class="t_list_caption">单号/时间</td>
            ...
            <td width="8%" class="t_list_caption">代理(占成)退水%</td>
            <td width="8%" class="t_list_caption">总代理(占成)退水%</td>
            <td width="8%" class="t_list_caption">股东(占成)退水%</td>
            <td width="8%" class="t_list_caption">大股东(占成)退水%</td>
            <td class="t_list_caption">公司(占成)</td>
...
if ($result->num_rows == 0) {?>
        ...
        <tr <? if ($image['qx']==1) { ?>class="delete" <?}?>
            ...
            <td height="35" align="center" bordercolor="cccccc" bgcolor="#FFFFFF">
                <?=$image['kithe']?>
                <br />
                <font color="red"><?=$image['abcd']?>盘<?=$image['user_ds']?></font>
            </td>
            ...
            <td width="63" align="center" bordercolor="cccccc" bgcolor="#FFFFFF">
                <?=$image['dai']?>
                <br>
            ...
            <td width="76" align="center" bordercolor="cccccc" bgcolor="#FFFFFF">
                <?=$image['dagu']?>
                <br>(
                <?=round($image['daguzc'], 2)?>
                )<br>
                <?=$image['dagu_ds']?>
            </td>
            <td align="center" nowrap="nowrap" bordercolor="cccccc" bgcolor="#FFFFFF">公司<br>(
                <?=round($image['gszc'], 2)?>
                )
            </td>
            <td align="center" nowrap="nowrap" bordercolor="cccccc" bgcolor="#FFFFFF">
        ...
        <tr>
            <td height="25" colspan="14" bordercolor="cccccc" bgcolor="#FFFFFF">
                <table width="98%" border="0" align="center" cellpadding="2" cellspacing="0" bordercolordark="#FFFFFF"
                    ...
                    <tr>
                        <td height="26" align="center">
                            ...
                            <a href="main.php?action=real_list&uid=<?=$uid?>&sort=<?=$sort?>&orderby=<?=$orderby?>&username=<?=$username?>&page=1">首页</a>
                            <a href="main.php?action=real_list&uid=<?=$uid?>&sort=<?=$sort?>&orderby=<?=$orderby?>&username=<?=$username?>&page=<?=$pre?>">上一页</a>
                            <a href="main.php?action=real_list&uid=<?=$uid?>&sort=<?=$sort?>&orderby=<?=$orderby?>&username=<?=$username?>&page=<?=$next?>">下一页</a>
                            <a href="main.php?action=real_list&uid=<?=$uid?>&sort=<?=$sort?>&orderby=<?=$orderby?>&username=<?=$username?>&page=<?=$last?>">末页</a>
                            <select name="page2" onChange="location.href='main.php?action=real_list&uid=<?=$uid?>&sort=<?=$sort?>&orderby=<?=$orderby?>&username=<?=$username?>&page='+this.options[selectedIndex].value">
                                <?$i = 1; 
                              for (; $i <= $pagecount; ++$i) {?>
