...
<form action="main.php" method="get" name="myFORM" id="myFORM">
 ...
 <form action="main.php?action=list_sx&uid=<?=$uid?>&act=save&kithe=<?=$kithe?>" method="post" name="form1" >
<? include "zf.php";?>
</form></div>

<sCRIPT language=javascript>makeRequest('main.php?action=server_sx&uid=<?=$uid?>&kithe=<?=$kithe?>&abcd=<?=$abcd?>&zc=<?=$zc?>')
</script>
<? if ($Current_KitheTable[29] != 0 && $Current_KitheTable[15] != 0 && $kithe == $Current_Kithe_Num) {?>
