...
<body style="min-width: 1200px;width: 100%">
 <div id="tit" class="tit" >
       ...
       <table width="874" border="0" cellspacing="0" cellpadding="0">
        ...
        <td height="28" style="color:#fff;">
            <font color="#CC0000">当前查询</font>-- <? if ($kithe != "") {?>第<?=$kithe?>期<?} else {?>日期区间：<?=$date_start?> ~<?=$date_end?>
				<?}?>
            &nbsp;&nbsp;类型:<? if ($class2 != "") { echo $class2;} else {echo  "全部";}?>
            &nbsp;&nbsp;注单状态:<? if ($wtype != "") {?> <?  if ($wtype == 1) {echo "注销";} else {echo"有效";}} else {echo  "全部";}?>
            &nbsp;&nbsp;大股东:<?=$user?>--<a href="javascript:history.go(-1);"><font color="ffffff">回上一页</font>
        </td>
        </tr>
        </tbody>
    </table>
    ...
    <?
    $result = $db1->query("SELECT  guan,sum(sum_m) as sum_m,sum(sz) as zs,sum(gszc) as gszc,sum(daguzc) as daguzc,sum(guanzc) as guanzc,sum(zongzc) as zongzc,sum(daizc) as daizc,sum(sum_m-sum_m*guan_ds/100) as sum_st FROM " . $tb_name . "  WHERE dagu='" . $user . "' and dai!='" . $user . "' and username!='" . $user . "' " . $vvv . " group by guan order by guan DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
     <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        <tr>
            <td colspan="10" align="center" nowrap="nowrap" class="t_list_caption">下级股东注单</td>
        </tr>
        <tr>
 ...
 <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
            ...
            <td align="center" nowrap="nowrap"><?=$rs['guan']?><?=$xm?>
            </td>
            <td align="center" nowrap="nowrap"><?=$rs['zs']?></td>
            <td align="center" nowrap="nowrap"><a href='main.php?action=real_list_m3&uid=<?=$uid?>&kithe=<?=$kithe?>&user=<?=$rs['guan']?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&wtype=<?=$wtype?>' ><?=round($rs['sum_m'], 2)?></a> </td>
...
if ($result->num_rows != 0) {?>
    ...
    <?}
$result = $db1->query("SELECT  username,sum(sum_m) as sum_m,sum(sz) as zs,sum(gszc) as gszc,sum(daguzc) as daguzc,sum(guanzc) as guanzc,sum(zongzc) as zongzc,sum(daizc) as daizc,sum(sum_m-sum_m*user_ds/100) as sum_st FROM " . $tb_name . "  WHERE lx=4 and username='" . $user . "' " . $vvv . " group by username order by username DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        <tr class="m_title_reall">
            <td colspan="6" align="center" nowrap="nowrap" class="t_list_caption">大股东走飞补货注单</td>
        </tr>
        <tr class="m_title_reall">
            <td width="54" align="center" nowrap="nowrap" class="t_list_tr_3">序号</td>
            <td width="133" align="center" class="t_list_tr_3">大股东</td>
        ...
        <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
            ...
            <td align="center" nowrap="nowrap"><a href='main.php?action=real_list_m6&uid=<?=$uid?>&kithe=<?=$kithe?>&user=<?=$rs['username']?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&wtype=<?=$wtype?>' >
                    <?=round($rs['sum_m'], 2)?>
                </a> </td>
            <td align="center" nowrap="nowrap">
                <?=round($rs['sum_st'], 2)?>
    ...
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        ...
        <tr class="t_list_tr_2">
            ...
            <td align="center" nowrap="nowrap">
                <?=round($z_zong1 + $z_zong2 + $z_zong3, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
