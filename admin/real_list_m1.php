...
<body style="min-width: 1200px;width: 100%">
 <div id="tit" class="tit" >
      ...
      <div class="biaoti_right floatright">
       <table width="874" border="0" cellspacing="0" cellpadding="0">
        ...
        <td height="28" style="color:#fff;">
            ...
            &nbsp;&nbsp;类型:<? if ($class2 != "") {?><?=$class2?><?} else {?>全部<?}?>
            &nbsp;&nbsp;注单状态:<? if ($wtype != "") {?><?if ($wtype == 1) {?>注销<?  } else {?>有效<?  }?>
            <?} else {?>全部<?}?>--<a href="javascript:history.go(-1);"><font color="ffffff">回上一页</font></a>
        </td>
        </tr>
        </tbody> 
    </table>
    ...
    <?$result = $db1->query("SELECT  dagu,sum(sum_m) as sum_m,sum(sz) as zs,sum(gszc) as gszc,sum(daguzc) as daguzc,sum(guanzc) as guanzc,sum(zongzc) as zongzc,sum(daizc) as daizc,sum(sum_m-sum_m*dagu_ds/100) as sum_st FROM " . $tb_name . "  WHERE dagu!='外调' " . $vvv . " group by dagu order by dagu DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        <tbody>
        <tr>
            <td colspan="10" align="center" nowrap="nowrap" class="t_list_caption">下级大股东注单</td>
        </tr>
        <tr>
            <td height="20" align="center" nowrap="nowrap" class="t_list_tr_3">序号</td>
        ...
        <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
            ...
            <td align="center" nowrap="nowrap"><?=$rs['dagu']?><?=$xm?></td>
            <td align="center" nowrap="nowrap"><?=$rs['zs']?></td>
            <td align="center" nowrap="nowrap"><a href='main.php?action=real_list_m2&uid=<?=$uid?>&kithe=<?=$kithe?>&user=<?=$rs['dagu']?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&wtype=<?=$wtype?>'><?=round($rs['sum_m'], 2)?></a> </td>
        ...
        <tr class="t_list_tr_2">
            ...
            <td align="center" nowrap="nowrap"><?=round($z_st1, 21)?></td>
    ...
    <?}
$result = $db1->query("SELECT  username,sum(sum_m) as sum_m,sum(sz) as zs,sum(gszc) as gszc,sum(daguzc) as daguzc,sum(guanzc) as guanzc,sum(zongzc) as zongzc,sum(daizc) as daizc,sum(sum_m-sum_m*user_ds/100) as sum_st  FROM " . $tb_name . " where lx=5 and dagu='外调' " . $vvv . " group by username order by username DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        <tbody>
            <td colspan="6" align="center" nowrap="nowrap" class="t_list_caption">公司外调补货注单</td>
        </tr>
        <tr class="m_title_reall">
            <td width="54" height="20" align="center" nowrap="nowrap" class="t_list_tr_3">序号</td>
            <td width="133" align="center" class="t_list_tr_3">公司走飞用户</td>
    ...
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        ...
        <tr class="t_list_tr_2">
            <td align="center" nowrap="nowrap"><?=$z_zs1 + $z_zs2?>
            </td>
            <td align="center" nowrap="nowrap"><?=round($z_sum1 + $z_sum2, 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($z_st1 + $z_st2, 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($z_gs1 + $z_gs2, 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($z_dagu1 + z_dagu2, 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($z_guan1 + $z_guan2, 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($z_zong1 + $z_zong2, 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($z_dai1 + $z_dai2, 2)?></td>
        </tr>
    </table>
    <?} else {?>
   <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
