...
$result = $db1->query("SELECT ds_lb,yg,rake_sj,zf_sum FROM web_config_ds WHERE class='ÌØÂë' Order By ID");
$ds_temp = array();
$x = 0;
...
$result = $db1->query("select class3,class1,class2,rate,blrate,ds_lb  from web_bl where class1='°ë²¨' order by id");
$ii = 1;
$list_table = array();
$Rs4 = array();
$Rs5 = array();
$result5 = $db1->query("SELECT class3,sum(if(lx=5 and dagu='Íâµ÷',sum_m,0)) as sum_wai,sum(" . $tb_zd . ") as sum_sum,sum(sz) as zs,sum(" . $tb_zd . "*rate) as win,sum(" . $tb_zd . "*Abs(dagu_ds)/100) as ds FROM " . $tb_name . " WHERE qx=0 " . $vvv . " and class1='°ë²¨' group by class3 order by class3 DESC");
...
for (; $k <= $list_table_count; $k += 1) {
    ...
    $bl_str = "<a style=\"cursor:pointer\" onClick=\"UpdateRate('MODIFYRATE','bl" . $k . "','bl" . $k . "','uid=" . $uid . "&class1=°ë²¨&ids=°ë²¨&sqq=sqq&lxlx=0&qtqt=" . $ds_temp[$list_table[$k]['ds_lb']]['rake_sj'] . "&class3=" . $list_table[$k]['num'] . "');\"><img src=\"images/sub.png\" width=\"19\" height=\"17\" border=\"0\" ></a>" . $bl_str . "<a style=\"cursor:pointer\" onClick=\"UpdateRate('MODIFYRATE','bl" . $k . "','bl" . $k . "','uid=" . $uid . "&class1=°ë²¨&ids=°ë²¨&sqq=sqq&lxlx=1&qtqt=" . $ds_temp[$list_table[$k]['ds_lb']]['rake_sj'] . "&class3=" . $list_table[$k]['num'] . "');\"><img src=\"images/add.png\" width=\"19\" height=\"17\" border=\"0\" ></a>";
    ...
    if ($list_table[$k]['zf'] == 0) {
        $zf_str = "<button class=btn4  onmouseover=this.className='btn4';return true; onMouseOut=this.className='btn4';return true; onclick=show_win('" . $list_table[$k]['num'] . "','" . $list_table[$k]['zf'] . "','" . $list_table[$k]['bl'] . "','" . $ds_temp[$list_table[$k]['ds_lb']]['zf_ds'] . "','" . $list_table[$k]['class1'] . "','" . $list_table[$k]['class2'] . "') >ÊÖ²¹</button>";
    } else {
        $zf_str = "<button class=btn4  onmouseover=this.className='btn4';return true; onMouseOut=this.className='btn4';return true; onclick=show_win('" . $list_table[$k]['num'] . "','" . $list_table[$k]['zf'] . "','" . $list_table[$k]['bl'] . "','" . $ds_temp[$list_table[$k]['ds_lb']]['zf_ds'] . "','" . $list_table[$k]['class1'] . "','" . $list_table[$k]['class2'] . "') >ÊÖ²¹ " . $list_table[$k]['zf'] . "</button>";
    }
    $blbl .= $num_str . "@@@" . $list_table[$k]['num'] . "@@@" . $bl_str . "@@@" . $list_table[$k]['zs'] . "@@@" . $list_table[$k]['sum'] . "@@@" . $list_table[$k]['win'] . "@@@" . $zf_str . "@@@" . $list_table[$k]['zzf'] . "###";
}
$blbl .= "&nbsp;@@@&nbsp;@@@&nbsp;@@@<font color=ff0000>" . $z_zs . "×¢</font>@@@<font color=ff0000>" . $z_sum . "</font>@@@&nbsp;@@@&nbsp;@@@<font color=ff0000>" . $z_zfsum . "</font>###";
echo $blbl;