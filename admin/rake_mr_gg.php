...
if ($_GET['act'] == "edit") {
    ...
    for (; $tt <= 42; ++$tt) {
        if (empty($_POST["Num_" . $tt])) {
            echo "<script>alert('赔率不能为空:" . $_POST["Num_" . $tt] . "/" . $tt . "!');window.location.href='main.php?action=rake_mr_gg&uid={$uid}&ids=" . $ids . "';</script>";
            exit;
        }
    }
    $tt = 1;
    ...
    echo "<script>alert('修改成功!');window.location.href='main.php?action=rake_mr_gg&uid={$uid}&ids=" . $ids . "';</script>";
    exit;
}
$result = $db1->query("select mrate,class3,class2 from web_bl where class1='过关'   Order By ID");
$showtable = array();
$y = 0;
...
<td valign="top" style="margin-top:2px;">
  <table width="100%" height="136" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list" >
        <form name="form1" method="post" action="main.php?action=rake_mr_gg&uid=<?=$uid?>&act=edit&ids=<?=$ids?>">
             <tbody>
            <? $B = 1;
for (; $B <= 2; $B += 1) {
    if ($B == 1) {?>
        ...
        for (; $I <= 7; $I += 1) {?>
            <tr>
                <td height="25" align="center" bgcolor="#FFFFFF">
                    <font color="<?=get_bs_css($showtable[$I - 1][1])?>">
                        <?=$showtable[$I - 1][1]?>
                    </font> <input name="class2_<?=$I?>" value="
            <?=$showtable[$I - 1][2]?>" type="hidden">
                </td>
                ...
                <td height="25" align="center" bgcolor="#FFFFFF">
                    <font color="<?=get_bs_css($showtable[$I + 7 - 1][1])?>">
                        <?=$showtable[$I + 7 - 1][1]?>
                    </font> <input name="class2_<?=$I + 7?>" value="<?=$showtable[$I + 7 - 1][2]?>" type="hidden">
                </td>
                <td height="25" align="center" bgcolor="#FFFFFF"><?=$showtable[$I + 7 - 1][0]?></td>
                <td height="25" align="center" bgcolor="#FFFFFF"><input style="HEIGHT: 18px" class="za_text"
                        maxlength="6" size="4" value="<?=$showtable[$I + 7 - 1][0]?>" name="Num_<?=$I + 7?>" /> <input name="class3_<?=$I + 7?>" value="<?=$showtable[$I + 7 - 1][1]?>" type="hidden"></td>

                <td height="25" align="center" bgcolor="#FFFFFF">
                    <font color="<?=get_bs_css($showtable[$I + 14 - 1][1])?>"><?=$showtable[$I + 14 - 1][1]?>
                    </font>

                    <input name="class2_<?=$I + 14?>" value="<?=$showtable[$I + 14 - 1][2]?>" type="hidden">
                </td>
                <td height="25" align="center" bgcolor="#FFFFFF"><?=$showtable[$I + 14 - 1][0]?></td>
                <td height="25" align="center" bgcolor="#FFFFFF"><input style="HEIGHT: 18px" class="za_text"
                        maxlength="6" size="4" value="<?=$showtable[$I + 14 - 1][0]?>" name="Num_<?=$I + 14?>" /><input name="class3_<?=$I + 14?>" value="<?=$showtable[$I + 14 - 1][1]?>" type="hidden"></td>
            </tr>
            <?}?>
            <? } else {?>
            <tr>
        ...
        for (; $I <= 7; $I += 1) {?>
            <tr>
                <td height="25" align="center" bgcolor="#FFFFFF">
                    <font color="<?=get_bs_css($showtable[$I + 21 - 1][1])?>">
                        <?=$showtable[$I + 21 - 1][1]?>
                    </font> <input name="class2_<?=$I + 21?>" value="<?=$showtable[$I + 21 - 1][2]?>" type="hidden">
                </td>
                <td height="25" align="center" bgcolor="#FFFFFF"><?=$showtable[$I + 21 - 1][0]?></td>
                <td height="25" align="center" bgcolor="#FFFFFF"><input style="HEIGHT: 18px" class="za_text"
                        maxlength="6" size="4" value="<?=$showtable[$I + 21 - 1][0]?>" name="Num_<?=$I + 21?>" />
                    <input name="class3_<?=$I + 21?>" value="<?=$showtable[$I + 21 - 1][1]?>" type="hidden">
                </td>
                <td height="25" align="center" bgcolor="#FFFFFF">
                    <font color="<?=get_bs_css($showtable[$I + 28 - 1][1])?>">
                        <?=$showtable[$I + 28 - 1][1]?>
                    </font> <input name="class2_<?=$I + 28?>" value="<?=$showtable[$I + 28 - 1][2]?>" type="hidden">
                </td>
                <td height="25" align="center" bgcolor="#FFFFFF"><?=$showtable[$I + 28 - 1][0]?></td>
                <td height="25" align="center" bgcolor="#FFFFFF"><input style="HEIGHT: 18px" class="za_text"
                        maxlength="6" size="4" value="<?=$showtable[$I + 28 - 1][0]?>" name="Num_<?=$I + 28?>" /> <input name="class3_<?=$I + 28?>" value="<?=$showtable[$I + 28 - 1][1]?>" type="hidden"></td>

                <td height="25" align="center" bgcolor="#FFFFFF">
                    <font color="<?=get_bs_css($showtable[$I + 35 - 1][1])?>"><?=$showtable[$I + 35 - 1][1]?>
                    </font>

                    <input name="class2_<?=$I + 35?>" value="<?=$showtable[$I + 35 - 1][2]?>" type="hidden">
                </td>
                <td height="25" align="center" bgcolor="#FFFFFF"><?=$showtable[$I + 35 - 1][0]?></td>
                <td height="25" align="center" bgcolor="#FFFFFF"><input style="HEIGHT: 18px" class="za_text"
                        maxlength="6" size="4" value="<?=$showtable[$I + 35 - 1][0]?>" name="Num_<?=$I + 35?>" /><input name="class3_<?=$I + 35?>" value="<?=$showtable[$I + 35 - 1][1]?>" type="hidden"></td>
            </tr>
            <?}
    }
}?>
