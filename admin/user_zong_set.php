...
$result = $db1->query("select * from web_agent where id=" . $_GET['id'] . " and lx=2 order by id");
...
$xm = $row['xm'];
$guan  = $row['guan'];
$result = $db1->query("select ds_id,yg,ygb,ygc,ygd,xx,xxx from  web_user_ds where lx=3 and ds_lock=0 and username='".$guan."' order by id");   
...
if ($_GET['act'] == "edit") {
    ...
    for ($I = 0; $I < $dsidcount; $I = $I + 1) {
		...
		//下级
		$db1->query("update web_user_ds set yg='".$yg1."' where yg>".$yg1." and ds_id='".$dsid_temp."' and  zong='".$username."'" );
		$db1->query("update web_user_ds set ygb='".$yg2."' where ygb>".$yg2." and ds_id='".$dsid_temp."' and  zong='".$username."'" );
		$db1->query("update web_user_ds set ygc='".$yg3."' where ygc>".$yg3." and ds_id='".$dsid_temp."' and  zong='".$username."'" );
		$db1->query("update web_user_ds set ygd='".$yg4."' where ygd>".$yg4." and ds_id='".$dsid_temp."' and  zong='".$username."'" );
		//单注单项
		$db1->query("update web_user_ds set xx='".$xx1."'  where xx>".$xx1." and ds_id='".$dsid_temp."' and  zong='".$username."'" );
		$db1->query("update web_user_ds set xxx='".$xxx1."'  where xxx>".$xxx1." and ds_id='".$dsid_temp."' and  zong='".$username."'" );
	}
	del_online($username,1);
	echo "<script>alert('设定成功!'); window.location.href = 'main.php?action=user_zong_set&uid=" . $uid . "&id=".$userid."';</script>";
	exit;
} 
?>
...
<body style="min-width: 1200px;width: 100%">
     <div id="tit" class="tit" >
      ...
      <div class="tit_center floatleft font_bold"><span class="font_g">总代理详细设定&nbsp;</span><span class="font_b">添加直属会员</span></div>
      <div class="tit_right floatleft"><img src="/images/tit_03.png" width="5" height="31" alt=""></div>
      <div class="biaoti_right floatright">
	 <table border="0" align="right" cellpadding="0" cellspacing="0">
		<tr>
			<td height="28" style="color:#fff;">&nbsp;总代理详细设定&nbsp;账号:
				<?=$username?> -- 名称:
	...
	<form action="main.php?action=user_zong_set&uid=<?=$uid?>&act=edit&id=<?=$userid?>" method="post" name="form1"
		...
		<table width="800" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list" >
			...
			<?
$result = $db1->query("select * from  web_user_ds where lx=2 and userid=" . $userid . " and username='".$username."' order by id ");
...
?>
		<tr bgcolor="#edf2f7">
				...
				<td align="center" >
					<input name="mmm<?=$t?>" class="za_text" id="mmm<?=$t?>" onkeyup="value=value.replace(/[^\d\.\/]/ig,'')"
