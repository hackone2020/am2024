...
$result = $db1->query("SELECT ds,yg,rake_sj,zf_sum FROM web_config_ds WHERE class='特码' Order By ID");
$ds_temp = array();
$x = 0;
...
$Rs4 = array();
$Rs5 = array();
$result5 = $db1->query("SELECT bl.class3,sum(if(lx=5 and dagu='外调',sum_m,0)) as sum_wai,sum(" . $tb_zd . ") as sum_sum,sum(sz) as zs,sum(" . $tb_zd . "*rate) as win,sum(" . $tb_zd . "*Abs(dagu_ds)/100) as ds FROM (SELECT class1,class3 FROM web_bl WHERE class1= '特码' AND (class2 = '特A' OR class2 = '特码') ORDER BY id) AS bl," . $tb_name . "  AS tan WHERE qx=0 and tan.class1 = bl.class1 and tan.class3=bl.class3 " . $vvv . " group by class3 order by class3 DESC");
...
for (; $k < $list_table_count; $k += 1) {
    ...
    $bl_str = "<a style=\"cursor:pointer\" onClick=\"UpdateRate('MODIFYRATE','bl" . $k . "','bl" . $k . "','uid=" . $uid . "&class1=特码&ids=特A&sqq=sqq&lxlx=0&qtqt=" . $ds_temp[get_st($list_table[$k]['num'])]['rake_sj'] . "&class3=" . $list_table[$k]['num'] . "');\"><img src=\"images\sub.png\" width=\"19\" height=\"19\" border=\"0\" ></a><span class=\"font_b\" style=\"margin:0 5px;\">" . $bl_str . "</span><a style=\"cursor:pointer\" onClick=\"UpdateRate('MODIFYRATE','bl" . $k . "','bl" . $k . "','uid=" . $uid . "&class1=特码&ids=特A&sqq=sqq&lxlx=1&qtqt=" . $ds_temp[get_st($list_table[$k]['num'])]['rake_sj'] . "&class3=" . $list_table[$k]['num'] . "');\"><img src=\"images/add.png\" width=\"19\" height=\"19\" border=\"0\" ></a>";
