    ...
    if ($_POST['sdel'] != "") {
        ...
        for (; $i < $del_num; ++$i) {
            ...
            if ($cou != 0) {
                ...
                $db1->query("Delete from web_user_ds where lx=1  and userid='{$sdel[$i]}'");
                $db1->query("Delete from web_user_ds where dai='{$kauser}'");
                $sql = "INSERT INTO  web_user_log set adddate='" . $utime . "',adduser='" . $jxadmin . "',kauser='" . $jxadmin . "',lx=3,text1='代理管理',text2='删除代理:" . $kauser . "',ip='" . $userip . "'";
                $db1->query($sql);
            }
        }
        echo "<script>alert('删除成功！');window.location.href='main.php?action=user_dai_list&uid={$uid}';</script>";
        exit;
    }
    if ($_GET['sdel'] != "") {
        $dell = $_GET['sdel'];
        ...
        if ($cou != 0) {
            ...
            $db1->query("Delete from web_agent_zi where guan='{$kauser}'");
            $db1->query("Delete from web_member where dai='{$kauser}'");
            $db1->query("Delete from web_user_ds where lx=1  and userid='{$sdel}'");
            $db1->query("Delete from web_user_ds where dai='{$kauser}'");
            $sql = "INSERT INTO  web_user_log set adddate='" . $utime . "',adduser='" . $jxadmin . "',kauser='" . $jxadmin . "',lx=3,text1='代理管理',text2='删除代理:" . $kauser . "',ip='" . $userip . "'";
            $db1->query($sql);
        }
...
if ($_GET['pz'] == "0") {
    ...
    $db1->query("update web_member set uid='' where dai='{$kauser}'");
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为开启走飞！!');window.location.href='main.php?action=user_dai_list&uid={$uid}';</script>";
    exit;
}
if ($_GET['pz'] == "1") {
    ...
    $db1->query("update web_agent_zi set uid='' where guan='{$kauser}'");
    $db1->query("update web_member set uid='' where dai='{$kauser}'");
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为禁止走飞!\\n该账号及其下属代理商将关闭走飞服务!!!');window.location.href='main.php?action=user_dai_list&uid={$uid}';</script>";
    exit;
}
if ($_GET['stat'] == "0") {
    ...
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为启用!');window.location.href='main.php?action=user_dai_list&uid={$uid}&enable=y';</script>";
    exit;
}
if ($_GET['stat'] == "1") {
    ...
    $db1->query("update web_agent_zi set uid='' where guan='{$kauser}'");
    $db1->query("update web_member set uid='' where dai='{$kauser}'");
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为停用!');window.location.href='main.php?action=user_dai_list&uid={$uid}&enable=n';</script>";
    exit;
}
if ($_GET['ty'] == "0") {
    ...
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为开放投注!');window.location.href='main.php?action=user_dai_list&uid={$uid}&enable=all';</script>";
    exit;
}
if ($_GET['ty'] == "1") {
    ...
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为暂停投注!');window.location.href='main.php?action=user_dai_list&uid={$uid}&enable=s';</script>";
    exit;
}
$sort = $_GET['sort'];
...
<style type="text/css">
    <!--
    .m_title_sucor {Background-color: #86C0A6;text-align: center}
    .m_title {background-color: 4B8E6F;text-align: center}
    -->
    </style>
    ...
    <tr>
        ...
        大股东:
            ...
            while ($image = $result->fetch_array()) {?>
                <option value="<?=$image['kauser']?>"><?=$image['kauser']?>
                </option>   
            <?}?>
            </select>
        股东:
        ...
        --
        <input type="button" name="append" value="新增" onClick="javascript:location.href='main.php?action=user_dai_add&uid=<?=$uid?>'" class="btn2" />
...
<form name="form1" method="post" action="main.php?action=user_dai_list&uid=<?=$uid?>">
<tbody> 
<tr>
<? if (strpos($flag, "13")) {?>
    <td width="20" class="t_list_caption">
	<input type="checkbox" name="sele" value="checkbox" onClick="javascript:checksel(this.form);" /></td>
<?}?>
...
<?$cou = $result->num_rows;
if ($cou == 0) {?>
    <tr class="m_title"><td height="30"  colspan="100%" class="t_list_tr_2" >        查无任何资料      </td>    </tr>
<?} else {
    while ($image = $result->fetch_array()) {
        $mumu = 0;
       ...
       <tr bgcolor="#ffffff" onmouseover="onMouseOver(this)" onmouseout="onMouseOut(this)" onclick="onMouseClick(this)">
		...
		<a href="main.php?action=user_mem_list&uid=<?=$uid?>&dai=<?=$image['kauser']?>"><?=$image['kauser']?></a></td>
		...
		<td width="50" align="center" nowrap><a href="main.php?action=user_dai_list&uid=<?=$uid?>&zong=<?=$image['zong']?>" style="color:000000" >(<?=$image['zong']?>)
        <?=$image['zong_zc']?>
        </a></td>
		<td width="50" align="center" nowrap><a href="main.php?action=user_dai_list&uid=<?=$uid?>&guan=<?=$image['guan']?>" style="color:000000" >(<?=$image['guan']?>)<?=$image['guan_zc']?>
        </a></td>
		<td width="50" height="25" align="center" nowrap>
		    <a href="main.php?action=user_dai_list&uid=<?=$uid?>&dagu=<?=$image['dagu']?>" style="color:000000" >(<?=$image['dagu']?>)<?=$image['dagu_zc']?></a>
		</td>
		<td width="50" height="25" align="center" nowrap><?=$image['gs_zc']?>
		...
		<a href="main.php?action=user_dai_edit&uid=<?=$uid?>&id=<?=$image['id']?>">修改资料</a> / 
		<a href="main.php?action=user_dai_set&uid=<?=$uid?>&id=<?=$image['id']?>">详细设置</a> / 
       <? if ($image['stat'] == 0) {?>
            <a href="main.php?action=user_dai_list&uid=<?=$uid?>&stat=1&name=<?=$image['kauser']?>&id=<?=$image['id']?>">停用</a>
        <?} else {?>
            <a href="main.php?action=user_dai_list&uid=<?=$uid?>&stat=0&name=<?=$image['kauser']?>&id=<?=$image['id']?>">
            <SPAN STYLE="color: rgb(255,0,0);">启用</SPAN></a>
        <?}?>
         / 
        <? if ($image['ty'] == 0) {?>
            <a href="main.php?action=user_dai_list&uid=<?=$uid?>&ty=1&name=<?=$image['kauser']?>&id=<?=$image['id']?>">暂停投注</a>
       <? } else {?>
            <a href="main.php?action=user_dai_list&uid=<?=$uid?>&ty=0&name=<?=$image['kauser']?>&id=<?=$image['id']?>">
       ...
       <? if ($image['pz'] == 0) {?>
            / <a href="main.php?action=user_dai_list&uid=<?=$uid?>&pz=1&name=<?=$image['kauser']?>&id=<?=$image['id']?>">禁止走飞</a>
        <?} else {?>
            / <a href="main.php?action=user_dai_list&uid=<?=$uid?>&pz=0&name=<?=$image['kauser']?>&id=<?=$image['id']?>">
       ...
       <?  if (strpos($flag, "13")) {?>
            / <a onClick="return confirm('此操作不可恢复!\n确定删除?');" href="main.php?action=user_dai_list&uid=<?=$uid?>&act=del&id=<?=$image['id']?>&sdel=<?=$image['id']?>">删除</a>
        <?}?>
        </td>    </tr>
    <?}?>
   <tr>
   ...
   <? if (strpos($flag, "13")) {?>
	...
	<td height="26" align="center" >
	    <span class="STYLE9"><?=$page?>页&nbsp;总<?=$pagecount?>页&nbsp;共<?=$total?>记录</span>&nbsp;
	  <a href="main.php?action=user_dai_list&uid=<?=$uid?>&zong=<?=$zong?>&guan=<?=$guan?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=1">首页</a>
	  <a href="main.php?action=user_dai_list&uid=<?=$uid?>&zong=<?=$zong?>&guan=<?=$guan?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=<?=$pre?>">上一页</a> 
	  <a href="main.php?action=user_dai_list&uid=<?=$uid?>&zong=<?=$zong?>&guan=<?=$guan?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=<?=$next?>">下一页</a> 
	  <a href="main.php?action=user_dai_list&uid=<?=$uid?>&zong=<?=$zong?>&guan=<?=$guan?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=<?=$last?>">末页</a></td>
	</tr></table></td>    </tr>
<?}?>
       </form>
       </tbody>   
...
<td align="right" valign="top" ><a style="cursor:hand;" onClick="closeSearchDlg();"><img src="images/edit_dot.gif" width="16" height="14"></a>
</td></tr>
</table></td></tr>
<tr>
<td bgcolor="#FFFFFF">
...
<option label="名称" value="name">名称</option></select></td>
</tr>
<tr bgcolor="#000000">
<td colspan="2" height="1"></td>
</tr>
<tr bgcolor="#A4C0CE"><td></td>
...
<input type="submit" id="dlg_ok" value="查询" class="za_button" onClick="submitSearchDlg();"></td></tr></table></td>
</tr></table></div></body>