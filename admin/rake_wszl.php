...
if ($_GET['act'] == "edit") {
    ...
    for (; $tt <= 10; ++$tt) {
        if (empty($_POST["lm" . $tt])) {
            echo "<script>alert('赔率不能为空:" . $_POST["Num_" . $tt] . "/" . $tt . "!');window.location.href='main.php?action=rake_wszl&uid={$uid}&ids=" . $ids . "';</script>";
            exit;
        }
    }
    $tt = 1;
    for (; $tt <= 10; ++$tt) {
        ...
        $db1->query("update web_bl  set adddate='" . $utime . "',rate=" . $num . " where class1='尾数量' and class2='" . $class2 . "' and  class3='" . $class3 . "'");
    }
    echo "<script>alert('修改成功!');window.location.href='main.php?action=rake_wszl&uid={$uid}&ids=" . $ids . "';</script>";
    exit;
}
$result = $db1->query("Select rate,class3,class2,locked from web_bl where class1='尾数量' and class2='" . $ids . "' Order By ID");
$showtable = array();
$y = 0;
...
$sx_count = $x - 1;
$result = $db1->query("SELECT ds,rake_sj FROM web_config_ds WHERE class='尾数量'  Order By ID");
$rake_sj_temp = array();
$x = 0;
while ($Image = $result->fetch_array()) {
            ...
            <tr><!--left-->
                ...
                <td valign="top" style="margin-top:2px;">
                    <table width="100%" border="0" cellpadding="0" cellspacing="1" class="t_list bet">
                        <form name="form1" method="post" action="main.php?action=rake_wszl&uid=<?= $uid ?>&act=edit&ids=<?= $ids ?>">
                            <tbody>
                                <tr>
