    ...
    if ($_POST['sdel'] != "") {
        ...
        for (; $i < $del_num; ++$i) {
            ...
            if ($cou != 0) {
                ...
                $sql = "INSERT INTO  web_user_log set adddate='" . $utime . "',adduser='" . $jxadmin . "',kauser='" . $jxadmin . "',lx=3,text1='会员管理',text2='删除会员:" . $kauser . "',ip='" . $userip . "'";
                $db1->query($sql);
            }
        }
        echo "<script>alert('删除成功！');window.location.href='main.php?action=user_mem_list&uid={$uid}';</script>";
        exit;
    }
    if ($_GET['sdel'] != "") {
        $dell = $_GET['sdel'];
...
if ($_GET['stat'] == "0") {
    ...
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为启用!'); window.location.href = 'main.php?action=user_mem_list&uid={$uid}&enable=y';</script>";
    exit;
}
if ($_GET['stat'] == "1") {
    ...
    $kauser = $_GET['name'];
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为停用!'); window.location.href = 'main.php?action=user_mem_list&uid={$uid}&enable=n';</script>";
    exit;
}
if ($_GET['ty'] == "0") {
    ...
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为开放投注!'); window.location.href = 'main.php?action=user_mem_list&uid={$uid}&enable=all';</script>";
    exit;
}
if ($_GET['ty'] == "1") {
    ...
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为暂停投注!'); window.location.href = 'main.php?action=user_mem_list&uid={$uid}&enable=s';</script>";
    exit;
}
$sort = $_GET['sort'];
...
while ($image = $result->fetch_array()) {?>
   ...
   <? }?>
                    ...
                    <input type="button" name="btn_search" value="快速查询" onClick="showSearchDlg();" class="za_button">
                            --
                    <input type="button" name="append" value="新增" onClick="javascript:location.href='main.php?action=user_mem_add&uid=<?=$uid?>'" class="btn2" />
   ...
   <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        <form name="form1" method="post" action="main.php?action=user_mem_list&uid=<?=$uid?>">
         <tbody>
            <tr >
                <? if (strpos($flag, "13")) {?>
...
if ($cou == 0) {?>
           ...
           <tr bgcolor="#ffffff" onmouseover="onMouseOver(this)" onmouseout="onMouseOut(this)" onclick="onMouseClick(this)">
                ...
                <td width="50" align="center" nowrap>
                    <a href="main.php?action=user_mem_list&uid=<?=$uid?>&dai=<?=$image['dai']?>" style="color:000000" >(<?=$image['dai']?>)<?=$image['dai_zc']?></a></td>
                <td width="50" align="center" nowrap>
                    <a href="main.php?action=user_mem_list&uid=<?=$uid?>&zong=<?=$image['zong']?>" style=" color:000000">(<?=$image['zong']?>)<?=$image['zong_zc']?></a>
                </td>
                <td width="50" align="center" nowrap>
                    <a href="main.php?action=user_mem_list&uid=<?=$uid?>&guan=<?=$image['guan']?>" style="color:000000">(<?=$image['guan']?>)<?=$image['guan_zc']?></a>
                </td>
                <td width="50" height="25" align="center" nowrap>
                    <a href="main.php?action=user_mem_list&uid=<?=$uid?>&dagu=<?=$image['dagu']?>" style="color:000000">(<?=$image['dagu']?>)<?=$image['dagu_zc']?></a></td>
                ...
                <td width="250" height="25" align="center">
                    <a href="main.php?action=user_mem_edit&uid=<?=$uid?>&id=<?=$image['id']?>">修改资料</a> / 
                    <a href="main.php?action=user_mem_set&uid=<?=$uid?>&id=<?=$image['id']?>">详细设置</a> /
                    <? if ($image['stat'] == 0) {?>
                    <a href="main.php?action=user_mem_list&uid=<?=$uid?>&stat=1&name=<?=$image['kauser']?>&id=<?=$image['id']?>">停用</a>
                    <?} else {?>
                    <a href="main.php?action=user_mem_list&uid=<?=$uid?>&stat=0&name=<?=$image['kauser']?>&id=<?=$image['id']?>">
                        <SPAN STYLE="color: rgb(255,0,0);">启用</SPAN></a>
                    <? } ?>
                    /
                    <?if ($image['ty'] == 0) {?>
                    <a href="main.php?action=user_mem_list&uid=<?=$uid?>&ty=1&name=<?=$image['kauser']?>&id=<?=$image['id']?>">暂停投注</a>
                    <?} else {?>
                    <a href="main.php?action=user_mem_list&uid=<?=$uid?>&ty=0&name=<?=$image['kauser']?>&id=<?=$image['id']?>">

                        <SPAN STYLE="color: rgb(255,0,0);">开放投注</SPAN></a>
                    <? } ?>
                    <br>
                    <a href="main.php?action=user_tab&uid=<?=$uid?>&username=<?=$image['kauser']?>&userlx=1" target="_blank">登入</a>
                    <? if (strpos($flag, "11")) {?>
                    ...
                    <? if (strpos($flag, "13")) {?>
                    / <a onClick="return confirm('此操作不可恢复!\n确定删除?');" href="main.php?action=user_mem_list&uid=<?=$uid?>&act=del&id=<?=$image['id']?>&sdel=<?=$image['id']?>">删除</a>
                    <? } ?>
		</td></tr>
   <? }?>
            <tr>
...
<a href="main.php?action=user_mem_list&uid=<?=$uid?>&dai=<?=$dai?>&zong=<?=$zong?>&guan=<?=$guan?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=1">首页</a> 
<a href="main.php?action=user_mem_list&uid=<?=$uid?>&dai=<?=$dai?>&zong=<?=$zong?>&guan=<?=$guan?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=<?=$pre?>">上一页</a> 
<a href="main.php?action=user_mem_list&uid=<?=$uid?>&dai=<?=$dai?>&zong=<?=$zong?>&guan=<?=$guan?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=<?=$next?>">下一页</a> 
<a href="main.php?action=user_mem_list&uid=<?=$uid?>&dai=<?=$dai?>&zong=<?=$zong?>&guan=<?=$guan?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=<?=$last?>">末页</a>
                            </td>
                        </tr>
                    </table>
                </td>
