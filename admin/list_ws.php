...
<Td width="10">&nbsp;</Td>
    ...
    <DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
        ...
        <form action="main.php?action=list_ws&uid=<?=$uid?>&act=save&kithe=<?=$kithe?>" method="post" name="form1">
            <? include "zf.php"; ?>
        </form>
    </div>
    <SCRIPT language=javascript>makeRequest('main.php?action=server_ws&uid=<?=$uid?>&kithe=<?=$kithe?>&abcd=<?=$abcd ?>&zc=<?=$zc?>')</script>
    <? if ($Current_KitheTable[29] != 0 && $Current_KitheTable[15] != 0 && $kithe == $Current_Kithe_Num) {?>
