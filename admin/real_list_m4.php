...
<body style="min-width: 1200px;width: 100%">
 <div id="tit" class="tit" >
      ...
      <div class="biaoti_right floatright">
       <table width="874" border="0" cellspacing="0" cellpadding="0">
        ...
        <td height="28" style="color:#fff;">
            <font color="#CC0000">当前查询</font>--<? if ($kithe != "") {?>第<?=$kithe?>期<?} else {?>日期区间：
            <?=$date_start?>~<?=$date_end?><?}?>
            &nbsp;&nbsp;类型:
            <? if ($class2 != "") {
            echo $class2;
    ...
    echo "全部";
            ...
            &nbsp;&nbsp;总代理:
            <?=$user?>--<a href="javascript:history.go( -1 );"><font color="ffffff">回上一页</font>
        </td>
        </tr>
           </tbody>
    </table>
    </div></div>
    <? $result = $db1->query("SELECT  dai,sum(sum_m) as sum_m,sum(sz) as zs,sum(gszc) as gszc,sum(daguzc) as daguzc,sum(guanzc) as guanzc,sum(zongzc) as zongzc,sum(daizc) as daizc,sum(sum_m-sum_m*dai_ds/100) as sum_st FROM " . $tb_name . "  WHERE zong='" . $user . "' and dai!='" . $user . "' and username!='" . $user . "' " . $vvv . " group by dai order by dai DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        <tr>
            <td colspan="10" align="center" nowrap="nowrap" class="t_list_caption">下级代理注单</td>
        </tr>
        <tr>
        ...
        <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
            <td height="22" align="center" nowrap="nowrap"><?=$ii?></td>
            <td align="center" nowrap="nowrap"><?=$rs['dai']?><?=$xm?></td>
            <td align="center" nowrap="nowrap"><?=$rs['zs']?></td>
            <td align="center" nowrap="nowrap"><a href='main.php?action=real_list_m5&uid=<?=$uid?>&kithe=<?=$kithe?>&user=<?=$rs['dai']?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&wtype=<?=$wtype?>' ><?=round($rs['sum_m'], 2)?></a> </td>
        ...
        <tr class="t_list_tr_2">
            ...
            <td align="center" nowrap="nowrap"><?=$z_zs1?>
            </td>
...
if ($result->num_rows!= 0) {?>
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        ...
        <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
            ...
            <td align="center" nowrap="nowrap"><?=$rs['zs']?></td>
            <td align="center" nowrap="nowrap"><a href='main.php?action=real_list_m6&uid=<?=$uid?>&kithe=<?=$kithe?>&user=<?=$rs['username']?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&wtype=<?=$wtype?>'><?=round($rs['sum_m'], 2)?>
                </a> </td>
            <td align="center" nowrap="nowrap"><?=round($rs['sum_st'], 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($rs['gszc'], 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($rs['daguzc'], 2)?></td>
    ...
    <?}
$result = $db1->query("SELECT  username,sum(sum_m) as sum_m,sum(sz) as zs,sum(gszc) as gszc,sum(daguzc) as daguzc,sum(guanzc) as guanzc,sum(zongzc) as zongzc,sum(daizc) as daizc,sum(sum_m-sum_m*user_ds/100) as sum_st FROM " . $tb_name . "  WHERE lx=2 and username='" . $user . "' " . $vvv . " group by username order by username DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        <tr>
            <td colspan="8" align="center" nowrap="nowrap" class="t_list_caption">总代理走飞补货注单</td>
        </tr>
        <tr>
            <td width="54" align="center" nowrap="nowrap" class="t_list_tr_3">序号</td>
            <td width="133" align="center" class="t_list_tr_3">总代理</td>
            ...
            <td width="251" align="center" class="t_list_tr_3">股东占成</td>
        ...
        <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
            ...
            <td align="center" nowrap="nowrap"><?=$rs['zs']?></td>
            <td align="center" nowrap="nowrap"><a href='main.php?action=real_list_m6&uid=<?=$uid?>&kithe=<?=$kithe?>&user=<?=$rs['username']?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&wtype=<?=$wtype?>' ><?=round($rs['sum_m'], 2)?></a> </td>
            <td align="center" nowrap="nowrap"><?=round($rs['sum_st'], 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($rs['guanzc'], 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($rs['daguzc'], 2)?></td>
            <td align="center" nowrap="nowrap"><?=round($rs['gszc'], 2)?></td>
        ...
        <tr class="t_list_tr_2">
            ...
            <td align="center" nowrap="nowrap"><?=round($z_guan3, 2)?></td>
...
if ($z_zs1 != 0 || $z_zs2 != 0 || $z_zs3 != 0) {?>
     <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        ...
        <tr class="t_list_tr_2">
            ...
            <td align="center" nowrap="nowrap">
                <?=round($z_st1 + $z_st2 + $z_st3, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=round($z_gs1 + $z_gs2 + $z_gs3, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=round($z_dagu1 + $z_dagu2 + $z_dagu3, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=round($z_guan1 + $z_guan2 + $z_guan3, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=round($z_zong1 + $z_zong2 - $z_sum3, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=round($z_dai1 + $z_dai2 + $z_dai3, 2)?>
            </td>
        </tr>
    </table><br>
    <?} else {?>
