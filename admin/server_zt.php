...
$result = $db1->query("SELECT ds,yg,rake_sj,zf_sum FROM web_config_ds WHERE class='正码1-6' Order By ID");
$ds_temp = array();
$x = 0;
...
$result = $db1->query("select class3,class1,class2,rate,blrate   from  web_bl where  class1='正特' and class2='" . $ids . "' order by id");
$ii = 1;
$list_table = array();
$Rs4 = array();
$Rs5 = array();
$result5 = $db1->query("SELECT class3,sum(if(lx=5 and dagu='外调',sum_m,0)) as sum_wai,sum(" . $tb_zd . ") as sum_sum,sum(sz) as zs,sum(" . $tb_zd . "*rate) as win,sum(" . $tb_zd . "*Abs(dagu_ds)/100) as ds FROM " . $tb_name . " WHERE qx=0 " . $vvv . " and class1='正特' and class2='" . $ids . "' group by class3 order by class3 DESC");
