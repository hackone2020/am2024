...
if ($_GET['act'] == "edit") {
    ...
    for (; $tt <= 49; ++$tt) {
        if (empty($_POST["Num_" . $tt])) {
            echo "<script>alert('赔率不能为空:" . $tt . "!');window.location.href='main.php?action=rake_mr_dzy&uid={$uid}&ids=" . $ids . "';</script>";
            exit;
        }
    }
    $tt = 1;
    for (; $tt <= 49; ++$tt) {
        ...
        $db1->query("update web_bl  set mrate=" . $num . " where class1='多中一' and class2='" . $ids . "' and  class3='" . $class3 . "'");
    }
    echo "<script>alert('修改成功!');window.location.href='main.php?action=rake_mr_dzy&uid={$uid}&ids=" . $ids . "';</script>";
    exit;
}
$result = $db1->query("Select mrate,class3,locked from web_bl where class1='多中一' and class2='" . $ids . "' Order By ID  LIMIT 49");
$showtable = array();
$y = 0;
...
function quick0() { var mm = $("money").value; for (var i = 1; i < 50; i++) { $("Num_" + i).value = mm; } }</script>
<body>
...
<td valign="top" style="margin-top:2px;">
  <table width="100%" height="136" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list" >
        <form name="form1" method="post" action="main.php?action=rake_mr_dzy&uid=<?=$uid?>&act=edit&ids=<?=$ids?>">
      <tbody>
            <tr>
