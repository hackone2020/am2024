...
$result = $db1->query( "select * from web_agent where id=".$_GET['id']." and lx=4 order by id" );
...
$username = $row['kauser'];
$xm = $row['xm'];
$result = $db1->query( "select id,yg,ygb,ygc,ygd,xx,xxx from  web_config_ds where lx=0 order by id" );
...
if ( $_GET['act'] == "edit" )
{
    ...
    for ( ; $I < $dsidcount; $I = $I + 1 )
    {
        ...
        $db1->query( "update web_user_ds set yg='".$yg1."' where yg>".$yg1." and ds_id='".$dsid_temp."' and  dagu='".$username."'" );
        $db1->query( "update web_user_ds set ygb='".$yg2."' where ygb>".$yg2." and ds_id='".$dsid_temp."' and  dagu='".$username."'" );
        $db1->query( "update web_user_ds set ygc='".$yg3."' where ygc>".$yg3." and ds_id='".$dsid_temp."' and  dagu='".$username."'" );
        $db1->query( "update web_user_ds set ygd='".$yg4."' where ygd>".$yg4." and ds_id='".$dsid_temp."' and  dagu='".$username."'" );
        $db1->query( "update web_user_ds set xx='".$xx1."'  where xx>".$xx1." and ds_id='".$dsid_temp."' and  dagu='".$username."'" );
        $db1->query( "update web_user_ds set xxx='".$xxx1."'  where xxx>".$xxx1." and ds_id='".$dsid_temp."' and  dagu='".$username."'" );
    }
    del_online( $username, 1 );
    echo "<script>alert('设定成功!');window.location.href='main.php?action=user_dagu_set&uid=".$uid."&id=".$userid."';</script>";
    exit( );
}
...
function CountGold(gold,type,rtype,bb,nmnm){
    ...
    <body style="min-width: 1200px;width: 100%">
     <div id="tit" class="tit" >
      ...
      <div class="tit_center floatleft font_bold"><span class="font_g">分公司详细设定&nbsp;</span><span class="font_b">添加直属会员</span></div>
      <div class="tit_right floatleft"><img src="/images/tit_03.png" width="5" height="31" alt=""></div>
      <div class="biaoti_right floatright">
          <table border="0" align="right" cellpadding="0" cellspacing="0">
            <tbody>
                <tr>
                <td height="28" style="color:#fff;">账号:<?=$username?>-- 名称:<?=$xm?>-- <input name="button" type="button" class="btn5" onmouseover="this.className='btn5m'" onmouseout="this.className='btn5'" onclick="history.go(-1)" value="回上一页"> &nbsp;&nbsp;</td>
                </tr>
            </tbody>
        </table>
                    
    </div></div>     
    <form action="main.php?action=user_dagu_set&uid=<?=$uid?>&act=edit&id=<?=$userid?>" method="post" name="form1" id="form1" onSubmit="return SubChk()">
       <table width="800" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
        <tbody> 
        <tr>
            <td width="100" class="t_list_tr_3">快速选单</td>
            <td width="646" class="t_list_tr_3">
                <input type="button" name="j1" id="j1"  value="降0.1"  onClick="ks(1,0.1)"/>
...
$result = $db1->query( "select * from  web_user_ds where lx=4 and userid=".$userid." and username='".$username."' order by id " );
...
<td width="100" height="26" align="right" ><?=$image['ds']?>:
    ...
    <input name="dsid[]" type="hidden" id="dsid[]" value="<?=$image['ds_id']?>" />            
</td>
<td align="center" bgcolor="#FFFFFF">
    <input name="yg[]" class="za_text" id="yg[]"  onkeyup="value=value.replace(/[^\d\.\/]/ig,'')" onBlur="return CountGold(this,'A','SP','mrds[<?=$t?>][1]','<?=$t?>');"  value='<?=$image['yg']?>' size="10" /></td>
<td align="center" bgcolor="#FFFFFF">
    <input name="ygb[]" class="za_text" id="ygb[]"  onkeyup="value=value.replace(/[^\d\.\/]/ig,'')" onBlur="return CountGold(this,'B','SP','mrds[<?=$t?>][2]','<?=$t?>');"  value='<?=$image['ygb']?>' size="10" />
</td>
<td align="center" bgcolor="#FFFFFF">
    <input name="ygc[]" class="za_text" id="ygc[]"  onkeyup="value=value.replace(/[^\d\.\/]/ig,'')" onBlur="return CountGold(this,'C','SP','mrds[<?=$t?>][3]','<?=$t?>');"  value='<?=$image['ygc']?>' size="10" />
</td>
<td align="center" bgcolor="#FFFFFF">
    <input name="ygd[]" class="za_text" id="ygd[]"  onkeyup="value=value.replace(/[^\d\.\/]/ig,'')" onBlur="return CountGold(this,'D','SP','mrds[<?=$t?>][4]','<?=$t?>');"  value='<?=$image['ygd']?>' size="10" />
</td>
<td align="center" bgcolor="#FFFFFF">
<input name="mm<?=$t?>" class="za_text" id="mm<?=$t?>" onkeyup="value=value.replace(/[^\d\.\/]/ig,'')"  onBlur="return CountGold(this,'blur','MP','mrds[<?=$t?>][5]','<?=$t?>');"  value='<?=$image['xx']?>' size="10" />           
</td>
<td align="center" bgcolor="#FFFFFF">
    <input name="mmm<?=$t?>" class="za_text" id="mmm<?=$t?>"  onkeyup="value=value.replace(/[^\d\.\/]/ig,'')"  onBlur="return CountGold(this,'blur','XP','mrds[<?=$t?>][6]','<?=$t?>');"  value='<?=$image['xxx']?>' size="10" />          
</td>
</tr>
<?
        $t++;
