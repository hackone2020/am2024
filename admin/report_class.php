...
$result = $db1->query("SELECT  ds_lb,sum(sum_m) as sum_m,sum(sz) as zs,sum(user_sf+dai_sq+zong_sq+guan_sq+dagu_sq) as usersf,sum(user_sq+dai_sq+zong_sq+guan_sq+dagu_sq) as user_ds,sum(gszc) as gszc,sum(gs_sf) as gssf FROM " . $tb_name . "  WHERE  dagu<>'外调' " . $vvv . " group by ds_lb order by ds_lb DESC");
$rs1_table = array();
while ($row = $result->fetch_assoc()) {
    $rs1_table[$row['ds_lb']] = $row;
}
$rs1_count = count($rs1_table);
$result = $db1->query("SELECT  ds_lb,sum(sum_m) as sum_m,sum(sz) as zs,sum(user_sf) as usersf,sum(user_sq) as user_ds FROM " . $tb_name . "  WHERE  dagu='外调' " . $vvv . " group by ds_lb order by ds_lb DESC");
$rs2_table = array();
while ($row = $result->fetch_assoc()) {
     ...
     <table border="0" align="right" cellpadding="0" cellspacing="0">
        <tbody>
         <tr>
            ...
            <font color="#CC0000">分类报表</font>-- <?if ($kithe != "") {?>第<?=$kithe?>期<?} else {?>日期区间：<?=$date_start?> ~<?=$date_end?> <?}?>
        ...
        if ($sum_m != 0) {
            ++$ii; ?>
                    <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
                        ...
                        <td align="center" nowrap="nowrap"><a href='main.php?action=report_m6&uid=<?=$uid?>&kithe=<?=$kithe?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$dstable[$i]['ds_lb']?>' >
                                <?=round($sum_m, 2)?>
                            </a></td>
                        <td height="28" align="center" nowrap="nowrap">
                            <?=round($usersf, 2)?>
                    ...
                    <tr class="t_list_tr_2">
                        ...
                        <td align="center" nowrap="nowrap"><?=round($z_gszc1, 2)?></td>
                        <td align="center" nowrap="nowrap"><?=round($z_gssf1, 2)?></td>
                    </tr>
                </table><br>
                <? }if (0 < $rs2_count) { ?>
                <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
                    ...
                    for (;$i < $ds_count; $i +=1) { 
                        ...
                        $z_userds2 +=$userds; $gssf=0 - $usersf;
                        $z_gssf2 +=$gssf; 
                        if ($sum_m !=0) { ++$ii;?>
                        <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
                        <td height="28" align="center" nowrap="nowrap">
                          <?=$ii?></td>
                        <td align="center" nowrap="nowrap"><?=$dstable[$i]['ds']?></td>
                        <td align="center" nowrap="nowrap"><?=$zs2?></td>
                        <td align="center" nowrap="nowrap"><a href='main.php?action=report_m6&uid=<?=$uid?>&kithe=<?=$kithe?>&date_start=<?=$date_start?>&date_end=<?$date_end?>&gtype=<?=$dstable[$i]['ds_lb']?>&lx=5' > <?=round($sum_m, 2)?></a></td>    
                       ...
                       ?> <tr class="t_list_tr_2">
                        ...
                        if (0 < $rs1_count || 0 < $rs2_count) { 
                            ...
                            <tr class="t_list_tr_2">
                                <td height="18" align="center" nowrap="nowrap"><?=round($z_zs1 + $z_zs2, 2)?></td>
                                <td align="center" nowrap="nowrap"><?=round($z_sum1 + $z_sum2, 2)?></td>
                                <td align="center" nowrap="nowrap"><?=round($z_usersf1 + $z_usersf2, 2)?></td>
                                <td align="center" nowrap="nowrap"><?=round($z_userds1 + $z_userds2, 2)?></td>
                                <td align="center" nowrap="nowrap"><?=round($z_gszc1 + $z_sum2, 2)?></td>
                                <td align="center" nowrap="nowrap"><?=round($z_gssf1 + $z_gssf2, 2)?></td>
                            </tr>
                            </table><br>
                            <?
                            } else {
                          ?><table width="100%" border="0" cellspacing="1" cellpadding="0">
                                ...
                                <tr>
                                    <td align=center height="20" bgcolor="#CCCCCC"><a
                                            href="javascript:history.go(-1);">离开</a></td>
                                </tr>
                            </table><?
                            }
                        ?></div>
