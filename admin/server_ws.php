...
$result = $db1->query("SELECT ds_lb,yg,rake_sj,zf_sum FROM web_config_ds WHERE class='尾数' Order By ID");
$ds_temp = array();
$x = 0;
...
$result = $db1->query("select class3,class1,class2,rate,blrate,ds_lb  from web_bl where class1='尾数' and class2='尾数' order by id");
$ii = 1;
$list_table = array();
$Rs4 = array();
$Rs5 = array();
$result5 = $db1->query("SELECT class3,sum(if(lx=5 and dagu='外调',sum_m,0)) as sum_wai,sum(" . $tb_zd . ") as sum_sum,sum(sz) as zs,sum(" . $tb_zd . "*rate) as win,sum(" . $tb_zd . "*Abs(dagu_ds)/100) as ds FROM " . $tb_name . " WHERE qx=0 " . $vvv . " and class1='尾数' and class2='尾数'  group by class3 order by class3 DESC");
...
for (; $k <= $list_table_count; $k += 1) {
    ...
    $num_color = "#CC0000";
    if ($na != 0 && ($list_table[$k]['num'] == $wsws1 || $list_table[$k]['num'] == $wsws2 || $list_table[$k]['num'] == $wsws3 || $list_table[$k]['num'] == $wsws4 || $list_table[$k]['num'] == $wsws5 || $list_table[$k]['num'] == $wsws6 || $list_table[$k]['num'] == $wsws0)) {
    ...
    $bl_str = "<a style=\"cursor:pointer\" onClick=\"UpdateRate('MODIFYRATE','bl" . $k . "','bl" . $k . "','uid=" . $uid . "&class1=尾数&ids=尾数&sqq=sqq&lxlx=0&qtqt=" . $ds_temp[$list_table[$k]['ds_lb']]['rake_sj'] . "&class3=" . $list_table[$k]['num'] . "');\"><img src=\"images/sub.png\" width=\"19\" height=\"17\" border=\"0\" ></a>" . $bl_str . "<a style=\"cursor:pointer\" onClick=\"UpdateRate('MODIFYRATE','bl" . $k . "','bl" . $k . "','uid=" . $uid . "&class1=尾数&ids=尾数&sqq=sqq&lxlx=1&qtqt=" . $ds_temp[$list_table[$k]['ds_lb']]['rake_sj'] . "&class3=" . $list_table[$k]['num'] . "');\"><img src=\"images/add.png\" width=\"19\" height=\"17\" border=\"0\" ></a>";
