...
<body style="min-width: 1200px;width: 100%">
 <div id="tit" class="tit" >
   ...
   echo "全部";
    ...
    <? $result = $db1->query("SELECT  username,sum(sum_m) as sum_m,sum(sz) as zs,sum(gszc) as gszc,sum(daguzc) as daguzc,sum(guanzc) as guanzc,sum(zongzc) as zongzc,sum(daizc) as daizc,sum(sum_m-sum_m*user_ds/100) as sum_st FROM " . $tb_name . "  WHERE dai='" . $user . "' and username!='" . $user . "' " . $vvv . " group by username order by username DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
   <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        <tr>
            <td colspan="10" align="center" nowrap="nowrap" class="t_list_caption">下级会员注单</td>
        </tr>
        <tr>
    ...
    <? }
$result = $db1->query("SELECT  username,sum(sum_m) as sum_m,sum(sz) as zs,sum(gszc) as gszc,sum(daguzc) as daguzc,sum(guanzc) as guanzc,sum(zongzc) as zongzc,sum(daizc) as daizc,sum(sum_m-sum_m*user_ds/100) as sum_st FROM " . $tb_name . "  WHERE lx=1 and username='" . $user . "' " . $vvv . " group by username order by username DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
        <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        <tr>
            <td colspan="9" align="center" nowrap="nowrap" class="t_list_caption">代理走飞补货注单</td>
        </tr>
        <tr >
            <td width="54" align="center" nowrap="nowrap" class="t_list_tr_3">序号</td>
            <td width="133" align="center" class="t_list_tr_3">代理</td>
            ...
            <td width="251" align="center" class="t_list_tr_3">总代理占成</td>
...
if ($z_zs1 != 0 || $z_zs2 != 0) {?>
       <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list bet">
        ...
        <tr class="t_list_tr_2">
            ...
            <td align="center" nowrap="nowrap">
                <?=round($z_dagu1 + $z_dagu2, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=round($z_guan1 + $z_guan2, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=round($z_zong1 + $z_zong2, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=round($z_dai1 + $z_dai2 - $z_sum3, 2)?>
            </td>
        </tr>
    </table>
    <? } else {?>
