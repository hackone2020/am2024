...
if ($_GET['act'] == "edit") {
    ...
    for (; $tt <= 10; ++$tt) {
        if (empty($_POST["lm" . $tt])) {
            echo "<script>alert('赔率不能为空:" . $_POST["Num_" . $tt] . "/" . $tt . "!');window.location.href='main.php?action=rake_mr_ws&uid={$uid}&ids=" . $ids . "';</script>";
            exit;
        }
    }
    $tt = 1;
    for (; $tt <= 10; ++$tt) {
        ...
        $db1->query("update web_bl  set mrate=" . $num . " where (class1='尾数' or class1='尾数连') and class2='" . $class2 . "' and  class3='" . $class3 . "'");
    }
    echo "<script>alert('修改成功!');window.location.href='main.php?action=rake_mr_ws&uid={$uid}&ids=" . $ids . "';</script>";
    exit;
}
$result = $db1->query("Select mrate,class3,class2 from web_bl where (class1='尾数' or class1='尾数连') and class2='" . $ids . "' Order By ID");
$showtable = array();
$y = 0;
            ...
            <tr><!--left-->
                ...
                <td valign="top" style="margin-top:2px;">
                    <table width="100%" height="136" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
                        <form name="form1" method="post" action="main.php?action=rake_mr_ws&uid=<?= $uid ?>&act=edit&ids=<?= $ids ?>">
                            <TBODY>
                                <TR align=center>
                                ...
                                <TR>
                                    <TD colspan="4" align="center" bgcolor="#FFFFFF">&nbsp;<font color="ff0000">统一修改</font>
