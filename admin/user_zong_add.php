...
if ($_GET['guan'] != "") {
    $guan = $_GET['guan'];
    $result = $db1->query("select * from web_agent where kauser='" . $guan . "' and lx=3 order by id LIMIT 1");
...
if ($_GET['act'] == "add") {
    ...
    $userid = $row['id'];
    $result = $db1->query("select id,class,ds_lb,ds_id,ds,yg,ygb,ygc,ygd,yge,ygf,ygg,ygh,xx,xxx,ds_lock from  web_user_ds where lx=3 and username='" . $guan . "' order by id");
    while ($image = $result->fetch_array()) {
        ...
        $db1->query("INSERT INTO web_user_ds set username='{$username}',userid='{$userid}',class='" . $image['class'] . "',ds_lb='" . $image['ds_lb'] . "',ds_id='" . $image['ds_id'] . "',ds='" . $image['ds'] . "',yg='" . $yg . "',ygb='" . $ygb . "',ygc='" . $ygc . "',ygd='" . $ygd . "',yge='" . $yge . "',ygf='" . $ygf . "',ygg='" . $ygg . "',ygh='" . $ygh . "',xx='" . $image['xx'] . "',xxx='" . $image['xxx'] . "',ds_lock='" . $image['ds_lock'] . "',lx='2',pz='0',pz_sum='0',zf_sum='500000',dagu='" . $dagu . "',guan='" . $guan . "',zong='',dai=''");
    }
    echo "<script>alert('添加成功!');window.location.href='main.php?action=user_zong_list&uid=" . $uid . "';</script>";
    exit;
} ?>
</script>
...
<body style="min-width: 1200px;width: 100%">
    ...
    <? if ($guan != "") { ?>
        ...
        <table width="800" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
            <tr>
                ...
                <td height="20" colspan="2" class="t_list_caption">股东(<?= $guan ?>)资料简述
                </td>
            </tr>
            <tr>
    ...
    <? } ?>
    <form name="form1" onSubmit="return SubChk()" method="post" action="main.php?action=user_zong_add&uid=<?= $uid ?>&act=add&guan=<?= $guan ?>">
        ...
        <table width="800" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
            ...
            <tr>
                ...
                <td height="30" bgcolor="#FFFFFF" class="t_edit_td">
                    <input name="agent" type="hidden" id="agent" value="<? if ($guan != "") { ?><?= $guan ?><? } ?>" />
                    <select id="guan" name="guan" onChange="var jmpURL='main.php?action=user_zong_add&uid=<?= $uid ?>&guan='+this.options[this.selectedIndex].value ; if(jmpURL!='')
                        {window.location=jmpURL;} else {this.selectedIndex=0 ;}">

                        <option value="">请选择上级账号</option>
                        <? $result = $db1->query("select * from web_agent where lx=3  order by id desc");
                        while ($image = $result->fetch_array()) { ?>
            ...
            <tr>
                ...
                <td height="30" bgcolor="#FFFFFF" class="t_edit_td">
                    ...
                    <input type="button" onClick="check_user();" value="检查账号" class="za_button" />&nbsp;帐号必须至少1个字元长，最多12个字ぃ?并只能有数??0-9)，及英文小写字母
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td height="30" colspan="2" align="center" bgcolor="#FFFFFF" class=""><input type="submit" class="btn2" name="button" id="button" value="确定" />
                </td>
            </tr>
        </table>
    </form>
