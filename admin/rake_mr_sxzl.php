...
if ($_GET['act'] == "edit") {
    ...
    for (; $tt <= 12; ++$tt) {
        ...
        $db1->query("update web_bl  set mrate=" . $num . " where class1='生肖量' and class2='" . $class2 . "' and  class3='" . $class3 . "'");
    }
    echo "<script>alert('修改成功!');window.location.href='main.php?action=rake_mr_sxzl&uid={$uid}&ids=" . $ids . "';</script>";
    exit;
}
$result = $db1->query("Select mrate,class3,class2 from web_bl where class1='生肖量' and class2='" . $ids . "' Order By ID");
$showtable = array();
$y = 0;
...
<td valign="top" style="margin-top:2px;">
  <table width="100%" height="136" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list" >
        <form name="form1" method="post" action="main.php?action=rake_mr_sxzl&uid=<?=$uid?>&act=edit&ids=<?=$ids?>">
            <tbody>
                <tr>
