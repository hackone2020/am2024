...
<script language="JavaScript">
    ...
    var url = "main.php?action=list_lm&class2=<?= $ids ?>&uid=<?= $uid ?>&kithe=<?= $kithe ?>&abcd=<?= $abcd ?>&zc=<?= $zc ?>";
</script>
...
<body style="min-width: 1200px;width: 100%">
    ...
    <DIV id="rs_window" style="DISPLAY: none; POSITION: absolute">
        <form action="main.php?action=list_lm&class2=<?= $ids ?>&uid=<?= $uid ?>&act=save&kithe=<?= $kithe ?>" method="post" name="form1">
            <? include "zf.php"; ?>
        </form>
    </div>

    <SCRIPT language="javascript">
        makeRequest('main.php?action=list_lm_show&class2=<?= $ids ?>&uid=<?= $uid ?>&kithe=<?= $kithe ?>&abcd=<?= $abcd ?>&zc=<?= $zc ?>&page=<?= $page ?>')
    </script>
    <? if ($Current_KitheTable[29] != 0 && $Current_KitheTable[15] != 0 && $kithe == $Current_Kithe_Num) { ?>
