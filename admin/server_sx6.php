...
$result = $db1->query("SELECT count(*) FROM (SELECT DISTINCT(class3) FROM " . $tb_name . " WHERE qx=0 " . $vvv . " and class1='生肖' and class2='六肖') AS temp_tan");
...
if ($num == 0) {?>
    ...
    <?} else {?>
    <? $result = $db1->query("SELECT class3,max(rate) as bl,sum(if(lx=5 and dagu='外调',sum_m,0)) as sum_wai,sum(" . $tb_zd . ") as sum_sum,sum(sz) as zs,sum(" . $tb_zd . "*rate) as win,sum(" . $tb_zd . "*Abs(dagu_ds)/100) as ds FROM " . $tb_name . " WHERE qx=0 " . $vvv . " and class1='生肖' and class2='六肖'  group by class3 ORDER BY sum_sum DESC limit " . $offset . "," . $pagesize);
    while ($image = $result->fetch_array()) {?>
    <tr bgcolor="#FFFFFF">
        <td height="28" align="center">
            <font size="3px" color="#CC0000"><b><?=$image['class3']?></b></font>
        </td>
        <td height="28" align="center"><?=$image['zs']?></td>
        <td height="28" align="center">
