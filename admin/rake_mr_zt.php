...
if ($_GET['act'] == "edit") {
	for ($tt = 1; $tt <= 49; $tt++) {
		if (empty($_POST['Num_' . $tt])) {
			echo "<script>alert('赔率不能为空:" . $tt . "!');window.location.href='main.php?action=rake_mr_zt&uid=$uid&ids=" . $ids . "';</script>";
			exit;
		} 
	} 

	if ($ids != "正1特" && $ids != "正2特" && $ids != "正3特" && $ids != "正4特" && $ids != "正5特" && $ids != "正6特") {
		$ids = "正1特";
	} 
	...
	echo "<script>alert('修改成功!');window.location.href='main.php?action=rake_mr_zt&uid=$uid&ids=" . $ids . "';</script>";
	exit;
} //结束


$result = $db1->query("Select mrate,class3,locked from web_bl where class1='正特'    and class2='" . $ids . "' Order By ID  LIMIT 49");
$showtable = array();
$y = 0;
...
<td valign="top" style="margin-top:2px;">
  <table width="100%" height="136" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list" >
        <form name="form1" method="post" action="main.php?action=rake_mr_zt&uid=<?=$uid?>&act=edit&ids=<?=$ids?>">
      <tbody>
            <tr>
