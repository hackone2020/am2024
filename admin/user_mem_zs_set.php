...
$result = $db1->query("select * from web_member where id=" . $_GET['id'] . " and zs>1 order by id");
...
$abcd = $row['abcd'];
$dai = $row['dai'];
$ahtml=$bhtml=$chtml=$dhtml="";
switch ($abcd){
   case "A":
		 $ahtml=" ";
		 $bhtml=$chtml=$dhtml=" readonly=\"readonly\" ";
		 break;
   case "B":
		 $bhtml=" ";
		 $ahtml=$chtml=$dhtml=" readonly=\"readonly\" ";
		 break;
   case "C":
		 $chtml=" ";
		 $ahtml=$bhtml=$dhtml=" readonly=\"readonly\" ";
		 break;
   case "D":
		 $dhtml=" ";
		 $ahtml=$bhtml=$chtml=" readonly=\"readonly\" ";
		 break;
   default:
		 $abcd = "A";
		 break;
...
$result = $db1->query("select ds_id,yg,ygb,ygc,ygd,xx,xxx from  web_user_ds where lx>1 and ds_lock=0 and username='".$dai."' order by id");   
...
if ($_GET['act'] == "edit") {
	...
	echo "<script>alert('设定成功!');window.location.href='main.php?action=user_mem_zs_set&uid=" . $uid . "&id=".$userid."';</script>";
	exit;
} 
?>
...
<body style="min-width: 1200px;width: 100%">
     <div id="tit" class="tit" >
      ...
      <div class="tit_center floatleft font_bold"><span class="font_g">大股东详细设定&nbsp;</span><span class="font_b">添加直属会员</span></div>
      <div class="tit_right floatleft"><img src="/images/tit_03.png" width="5" height="31" alt=""></div>
      <div class="biaoti_right floatright">
<table border="0" align="right" cellpadding="0" cellspacing="0">
		<tr>
			<td height="28" style="color:#fff;">&nbsp;直属会员详细设定&nbsp;账号:
				<?=$username?> -- 名称:<?=$xm?> -- <input name="button" type="button" class="btn5" onmouseover="this.className='btn5m'" onmouseout="this.className='btn5'" onclick="history.go(-1)" value="回上一页">&nbsp;&nbsp;
			</td>
		</tr>
	</table>
	</div></div>
	<form action="main.php?action=user_mem_zs_set&uid=<?=$uid?>&act=edit&id=<?=$userid?>" method="post" name="form1"
		...
		<table width="800" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
            ...
            <tr>
				...
				<td width="646" class="t_list_tr_3"><input type="button" name="j1" id="j1" value="降0.1" onClick="ks(1,0.1)" />
             ...
             <table width="800" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list" >
              <tbody>
                <tr>
				<td width="110" height="20" align="center" nowrap class="t_list_caption">类型</td>
				<td width="110" height="20" align="center" class="t_list_caption">A盘退水%</td>
				<td width="110" height="20" align="center" class="t_list_caption">B盘退水%</td>
				<td width="110" height="20" align="center" class="t_list_caption">C盘退水%</td>
				<td width="110" height="20" align="center" class="t_list_caption">D盘退水%</td>
				<td width="110" height="20" align="center" class="t_list_caption">单注限额</td>
				<td width="110" height="20" align="center" class="t_list_caption">单项(号)限额</td>
			</tr>
			<?
$result = $db1->query("select * from  web_user_ds where lx=0 and userid=" . $userid . " and username='".$username."' order by id ");
...
?>
				<tr bgcolor="#edf2f7">
				<td width="110" height="26" align="right" nowrap >
					<?=$image['ds']?>:
					<input name="ds[]" type="hidden" id="ds[]" value="<?=$image['ds']?>" />
					<input name="dsid[]" type="hidden" id="dsid[]" value="<?=$image['ds_id']?>" />
				</td>
				<td width="110" align="center" ><input name="yg[]" class="za_text" id="yg[]"
						onkeyup="value=value.replace(/[^\d\.\/]/ig,'')"
						onBlur="return CountGold(this,'A','SP','mrds[<?=$t?>][1]','<?=$t?>');" value='<?=$image['yg']?>' size="10" /></td>
				<td width="110" align="center" ><input name="ygb[]" class="za_text" id="ygb[]"
						onkeyup="value=value.replace(/[^\d\.\/]/ig,'')"
						onBlur="return CountGold(this,'B','SP','mrds[<?=$t?>][2]','<?=$t?>');" value='<?=$image['ygb']?>' size="10" /></td>
				<td width="110" align="center" ><input name="ygc[]" class="za_text" id="ygc[]"
						onkeyup="value=value.replace(/[^\d\.\/]/ig,'')"
						onBlur="return CountGold(this,'C','SP','mrds[<?=$t?>][3]','<?=$t?>');" value='<?=$image['ygc']?>' size="10" /></td>
				<td width="110" align="center" ><input name="ygd[]" class="za_text" id="ygd[]"
						onkeyup="value=value.replace(/[^\d\.\/]/ig,'')"
						onBlur="return CountGold(this,'D','SP','mrds[<?=$t?>][4]','<?=$t?>');" value='<?=$image['ygd']?>' size="10" /></td>
				<td width="110" align="center" >
					<input name="mm<?=$t?>" class="za_text" id="mm<?=$t?>"
						onkeyup="value=value.replace(/[^\d\.\/]/ig,'')"
						onBlur="return CountGold(this,'blur','MP','mrds[<?=$t?>][5]','<?=$t?>');" value='<?=$image['xx']?>' size="10" />
				</td>
				<td width="110" align="center" >
					<input name="mmm<?=$t?>" class="za_text" id="mmm<?=$t?>"
						onkeyup="value=value.replace(/[^\d\.\/]/ig,'')"
						onBlur="return CountGold(this,'blur','XP','mrds[<?=$t?>][6]','<?=$t?>');" value='<?=$image['xxx']?>' size="10" />
				</td>
			</tr>
			<? 
  $t++; 
