...
$result = $db1->query("select class3,class1,class2,rate,blrate,ds_lb  from web_bl where class1='正码1-6' order by id");
$ii = 1;
$list_table = array();
$Rs4 = array();
$Rs5 = array();
$result5 = $db1->query("SELECT class2,class3,sum(if(lx=5 and dagu='外调',sum_m,0)) as sum_wai,sum(" . $tb_zd . ") as sum_sum,sum(sz) as zs,sum(" . $tb_zd . "*rate) as win,sum(" . $tb_zd . "*Abs(dagu_ds)/100) as ds FROM " . $tb_name . " WHERE qx=0 " . $vvv . " and class1='正码1-6'  group by class2,class3 order by class2 DESC");
while ($row = $result5->fetch_assoc()) {
    $Rs5[$row['class2']][$row['class3']] = $row;
    $Rs4[$row['class2']][$row['class3']]['sum_sum'] = $row['sum_wai'];
}
...
while ($rs = $result->fetch_assoc()) {
    $sum_m = 0;
    $sum_m = round($Rs5[$rs['class2']][$rs['class3']]['sum_sum'], 2);
    $z_zs += $Rs5[$rs['class2']][$rs['class3']]['zs'];
    $z_sum += $sum_m;
    ...
    $list_table[$ii]['zs'] = $Rs5[$rs['class2']][$rs['class3']]['zs'];
    $list_table[$ii]['sum'] = $sum_m;
    $list_table[$ii]['ds'] = $Rs5[$rs['class2']][$rs['class3']]['ds'];
    $list_table[$ii]['win'] = round($Rs5[$rs['class2']][$rs['class3']]['win'], 2);
    $list_table[$ii]['zf'] = 0;
    if ($Rs4[$rs['class2']][$rs['class3']]['sum_sum'] != "") {
        $list_table[$ii]['zzf'] = abs($Rs4[$rs['class2']][$rs['class3']]['sum_sum']);
        $z_zfsum += abs($Rs4[$rs['class2']][$rs['class3']]['sum_sum']);
    } else {
        $list_table[$ii]['zzf'] = 0;
    }
    ++$ii;
...
for (; $k <= $list_table_count; $k += 1) {
    ...
    $num_color = get_bs_css($list_table[$k]['num']);
    if ($num_color == "") {
        $num_color = "#CC0000";
    }
    $num_str = "<font size=\"3px\" color=\"" . $num_color . "\">" . $list_table[$k]['num'] . "</font>";
    $bl_str = $list_table[$k]['bl'];
    $bl_str = "<a style=\"cursor:pointer\" onClick=\"UpdateRate('MODIFYRATE','bl" . $k . "','bl" . $k . "','uid=" . $uid . "&class1=正码1-6&ids=" . $list_table[$k]['class2'] . "&sqq=sqq&lxlx=0&qtqt=" . $ds_temp[$list_table[$k]['ds_lb']]['rake_sj'] . "&class3=" . $list_table[$k]['num'] . "');\"><img src=\"images/sub.png\" width=\"19\" height=\"17\" border=\"0\" ></a>" . $bl_str . "<a style=\"cursor:pointer\" onClick=\"UpdateRate('MODIFYRATE','bl" . $k . "','bl" . $k . "','uid=" . $uid . "&class1=正码1-6&ids=" . $list_table[$k]['class2'] . "&sqq=sqq&lxlx=1&qtqt=" . $ds_temp[$list_table[$k]['ds_lb']]['rake_sj'] . "&class3=" . $list_table[$k]['num'] . "');\"><img src=\"images/add.png\" width=\"19\" height=\"17\" border=\"0\" ></a>";
    ...
    $blbl .= $num_str . "@@@" . $list_table[$k]['num'] . "@@@" . $bl_str . "@@@" . $list_table[$k]['zs'] . "@@@" . $list_table[$k]['sum'] . "@@@" . $list_table[$k]['win'] . "@@@" . $zf_str . "@@@" . $list_table[$k]['zzf'] . "@@@" . $list_table[$k]['class2'] . "###";
}
