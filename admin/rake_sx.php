...
function get_rake_sj($i)
{
    ...
    switch ($i) {
        case "一肖":
            $rake_sj = $rake_sj_temp['一肖'];
            break;
        case "一肖不中":
            $rake_sj = $rake_sj_temp['一肖不中'];
            break;
        case "特肖":
            $rake_sj = $rake_sj_temp['特肖'];
            break;
        case "六肖":
            $rake_sj = $rake_sj_temp['六肖'];
            break;
        case "二肖连中":
            $rake_sj = $rake_sj_temp['二肖连'];
            break;
        case "二肖连不中":
            $rake_sj = $rake_sj_temp['二肖连'];
            break;
        case "三肖连中":
            $rake_sj = $rake_sj_temp['三肖连'];
            break;
        case "三肖连不中":
            $rake_sj = $rake_sj_temp['三肖连'];
            break;
        case "四肖连中":
            $rake_sj = $rake_sj_temp['四肖连'];
            break;
        case "四肖连不中":
            $rake_sj = $rake_sj_temp['四肖连'];
            break;
        case "五肖连中":
            $rake_sj = $rake_sj_temp['五肖连'];
            break;
        case "五肖连不中":
            $rake_sj = $rake_sj_temp['五肖连'];
            break;
...
if ($ids == "一肖" ||$ids == "一肖不中"|| $ids == "特肖" || $ids == "六肖") {
    $class1 = "生肖";
}
if ($ids == "二肖连中" || $ids == "二肖连不中" || $ids == "三肖连中" || $ids == "三肖连不中" || $ids == "四肖连中" || $ids == "四肖连不中" || $ids == "五肖连中" || $ids == "五肖连不中") {
    $class1 = "生肖连";
}
if ($_GET['act'] == "edit") {
    $tt = 1;
    for (; $tt <= 12; ++$tt) {
        if (empty($_POST["lm" . $tt])) {
            echo "<script>alert('赔率不能为空:" . $_POST["Num_" . $tt] . "/" . $tt . "!');window.location.href='main.php?action=rake_sx&uid={$uid}&ids=" . $ids . "';</script>";
            exit;
        }
    }
    $tt = 1;
    for (; $tt <= 12; ++$tt) {
        ...
        $db1->query("update web_bl  set adddate='" . $utime . "',rate=" . $num . " where (class1='生肖' or class1='生肖连') and class2='" . $class2 . "' and  class3='" . $class3 . "'");
    }
    echo "<script>alert('修改成功!');window.location.href='main.php?action=rake_sx&uid={$uid}&ids=" . $ids . "';</script>";
    exit;
}
$result = $db1->query("Select rate,class3,class2,locked from web_bl where (class1='生肖' or class1='生肖连') and class2='" . $ids . "' Order By ID");
$showtable = array();
$y = 0;
...
$sx_count = $x - 1;
$result = $db1->query("SELECT ds,rake_sj FROM web_config_ds WHERE class='生肖' or class='生肖连'  Order By ID");
$rake_sj_temp = array();
$x = 0;
while ($Image = $result->fetch_array()) {
            ...
            <tr><!--left-->
                ...
                <td valign="top" style="margin-top:2px;">
                    <table width="100%" border="0" cellpadding="0" cellspacing="1" class="t_list bet">
                        <form name="form1" method="post" action="main.php?action=rake_sx&uid=<?= $uid ?>&act=edit&ids=<?= $ids ?>">
                            <tbody>
                                <tr>
