<table  width="800" border="0" cellpadding="0" cellspacing="0">
   ...
   <table  width="800" border="0" cellpadding="0" cellspacing="1" class="t_list">
    <form target="leftFrame" name="form1" method="post" action="main.php?action=bet_n5&uid=<?=$uid?>&ids=<?=$ids?>&class2=<?=$class2?>" onSubmit="return ChkSubmit()">
     <tbody>
        <tr>
           <td height="27" colspan="15" align="center" class="t_list_tr_2">
            <INPUT name="rtype" type=radio onclick=go_main(url+"1"); value="1" <? if($lx=="1" ) { ?> checked <?}?>><span>五不中</span>
            <INPUT name="rtype" type=radio onclick=go_main(url+"2"); value="2" <? if($lx=="2" ) { ?> checked <?}?>><span>六不中</span>
            <INPUT name="rtype" type=radio onclick=go_main(url+"3"); value="3" <? if($lx=="3" ) { ?> checked <?}?>><span>七不中</span>
            <INPUT name="rtype" type=radio onclick=go_main(url+"4"); value="4" <? if($lx=="4" ) { ?> checked <?}?>><span>八不中</span>
            <INPUT name="rtype" type=radio onclick=go_main(url+"5"); value="5" <? if($lx=="5" ) { ?> checked <?}?>><span>九不中</span>
            <INPUT name="rtype" type=radio onclick=go_main(url+"6"); value="6" <? if($lx=="6" ) { ?> checked <?}?>><span>十不中</span>
            <INPUT name="rtype" type=radio onclick=go_main(url+"7"); value="7" <? if($lx=="7" ) { ?> checked <?}?>><span>十一不中</span>
            <INPUT name="rtype" type=radio onclick=go_main(url+"8"); value="8" <? if($lx=="8" ) { ?> checked <?}?>><span>十二不中</span>
            </td>
         </tr>
        <tr>
