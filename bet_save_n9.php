...
if ($sum_sum < 0 || $sum_sum == "" || !is_numeric($sum_sum) || $number1 == "" || $number2 == "" || 0 < !$sum_count) {
    echo "<script>alert('对不起,系统异常!');</script>";
    exit;
}
$XF = 15;
...
$number_array1 = explode(",", $number1);
$number_count1 = count($number_array1) - 1;
if ($number_count1 < $tm_type_min) {
    echo " <script Language=Javascript>alert('最少选择" . $tm_type_min . "个特码！');window.location.href='main.php?action=mem_left&uid=" . $uid . "';</script>";
    exit;
}
if ($tm_type_max < $number_count1) {
    echo "<script Language=Javascript>alert('最多选择" . $tm_type_max . "个特码！');window.location.href='main.php?action=mem_left&uid=" . $uid . "';</script>";
    exit;
}
if (count($number_array1) != count(array_unique($number_array1))) {
    echo "<script>alert('对不起,系统异常!'); window.open('index.php', '_top')</script>";
    exit;
}
$number_array2 = explode(",", $number2);
$number_count2 = count($number_array2) - 1;
if ($number_count2 < $zm_type_min) {
    echo "<script Language=Javascript>alert('最少选择" . $zm_type_min . "个正码！');window.location.href='main.php?action=mem_left&uid=" . $uid . "';</script>";
    exit;
}
if ($zm_type_max < $number_count2) {
    echo "<script Language=Javascript>alert('最多选择" . $zm_type_max . "个正码！');window.location.href='main.php?action=mem_left&uid=" . $uid . "';</script>";
    exit;
}
if (count($number_array2) != count(array_unique($number_array2))) {
    echo "<script>alert('对不起,系统异常!'); window.open('index.php', '_top')</script>";
    exit;
}
...
$rowm = mysql_fetch_assoc($resultmem);
$dai_zc = $rowm['dai_zc'];
...
for (; $r < $number_count; ++$r) {
    ...
    if (is_numeric($temename) && 1 <= $temename && $temename <= 49) {
        ...
        if ($bl_locked == 1) {
            ...
            $rate_temp[$temename] = $bl_temp[$temename]['rate'];
        }
    } else {
        echo "<script>alert('对不起,系统异常!'); window.open('index.php', '_top')</script>";
        exit;
...
$i = 0;
for (; $i < $number_count1; ++$i) {
    if ($number_array1[$i] != "") {
        $j = 0;
        for (; $j < $number_count2; ++$j) {
            if ($number_array1[$i] != $number_array2[$j]) {
                $number1[$zs] = "特:" . $number_array1[$i] . ",正:" . $number_array2[$j] . ",";
                $text1_temp[$zs] = "@|" . $number_array1[$i] . "||" . $number_array2[$j] . "|@";
                $bl_rate_temp[$zs] = min($rate_temp[$number_array1[$i]], $rate_temp[$number_array2[$j]]);
                $zs += 1;
            }
        }
    }
...
for (; $z <= $zs; ++$z) {
    ...
    if ($bl_class3 != "" && $bl_rate != "" && $text1 != "") {
        ...
        $sum_count += 1;
        $text2 .= "<tr><td>" . $bl_class2 . "</td><td>" . $bl_class3 . "</td><td><font color='green'>" . $rate0 . "</font></td><td><font color='green'>" . $sum_m . "</font></td>";
        $num = randstr();
...
<body>
    ...
    <table width="90%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list"  id="showtable2">
        <tbody bgcolor="#FFFFFF">
                ...
                <?$text2?>
                <tr>
                    <td colspan="4" align="right">共 <b id="sum">"
                            <?$sum_count?>
                 ...
                 <tr>
                <td style="padding-top: 8px; padding-bottom: 4px; "  colspan="4" bgcolor="#ffffff">
