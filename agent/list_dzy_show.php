...
<?} else {
    ...
    while ($image = $result->fetch_array()) {?>
          <tr bgcolor="#FFFFFF">    <td height="28" align="center"><?=$image['class3']?></td>
          <td height="28" align="center"><?=$image['zs']?></td>
          <td height="28" align="center">   <? $strtmp = "look.html?action=list_look&uid=" . $uid . "&vip=" . $vip . "&kithe=" . $kithe . "&class2=" . $class2 . "&class3=" . $image['class3'];?>
            <button class="btn1" onmouseover="this.className='btn2';return true;" onMouseOut="this.className='btn1';return true;" onClick="ops('<?=$strtmp?>
        ',400,750);"><?=round($image['sum_sum'], 2)?>
        </button>    </td>
        <td height="28" align="center"><?=round($image['win'], 2)?>
