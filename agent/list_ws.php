...
if ($_GET['act'] == "save") {
    if ($vip == 1) {
        echo "<script>alert('子账号不允许走飞!'); window.location.href = 'main.php?action=list_ws&uid=" . $uid . "&vip=" . $vip . "&ids=" . $ids . "';</script>";
        exit;
    }
    if ($pz_of == 1 || $ty == 1) {
        echo "<script>alert('对不起，您没有走飞功能或者系统停押!如有疑问请联系上级!'); window.location.href = 'main.php?action=list_ws&uid=" . $uid . "&vip=" . $vip . "&ids=" . $ids . "';</script>";
        exit;
    }
    if (empty($_POST['sum_m']) || $_POST['sum_m'] < 1) {
        echo "<script>alert('金额有误，请输入数字!');window.location.href='main.php?action=list_ws&uid=" . $uid . "&vip=" . $vip . "&ids=" . $ids . "';</script>";
        exit;
    }
    if ($Current_KitheTable[29] == 0 || $Current_KitheTable[15] == 0) {
        echo "<script>alert('该项目已经封盘!'); window.location.href = 'main.php?action=list_ws&uid=" . $uid . "&vip=" . $vip . "&ids=" . $ids . "';</script>";
        exit;
    }
...
<script src="js/list_ws.js" type="text/javascript"></script>
            ...
            <td colspan="7" align="center" class="t_list_caption">
             ...
             <table width="100%" border="0" cellpadding="0" cellspacing="1" >
                ...
                <tr>          
                    ...
                    <td width="560" align="right">&nbsp;盘口:&nbsp;</td>
                    <td>
                        ...
                        <input name="action" type="hidden" id="action" value="list_ws" />
                        <input name="uid" type="hidden" id="uid" value="<?=$uid?>" /><input name="vip" type="hidden"
                            id="vip" value="<?=$vip?>" />
                    <span>&nbsp;类型:&nbsp;</span>
...
<!--table1 end-->  
    ...
    <DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
        <form action="main.php?action=list_ws&uid=<?=$uid?>&vip=<?=$vip?>&act=save&kithe=<?=$kithe?>" method="post" name="form1" >
            <? include "zf.php";?>
        </form>
    </div>
    <SCRIPT language=javascript>makeRequest('main.php?action=server_ws&uid=<?=$uid?>&vip=<?=$vip?>&kithe=<?=$$kithe?>&abcd=<?=$$abcd?>&zc=<?=$$zc?>')</script>
    <?if ($Current_KitheTable[29] != 0 && $Current_KitheTable[15] != 0 && $kithe == $Current_Kithe_Num) {?>
