...
$result = $db1->query("select * from web_agent where id=" . $_GET['id'] . " and lx=1 " . $select_sql . " order by id LIMIT 1");
...
if ($zong != "") {
    $result1 = $db1->query("select * from web_agent where kauser='" . $zong . "' and lx=2 order by id LIMIT 1");
    $rows = $result1->fetch_assoc();
    if ($rows == "") {
        exit;
    }
    $maxcs = $rows['cs'];
    $zongmaxcs = $rows['cs'];
    $zongmaxzc = $rows['zong_zc'];
    $gs_zc = $rows['gs_zc'];
    $dagu_zc = $rows['dagu_zc'];
    $guan_zc = $rows['guan_zc'];
    $iend = $rows['zong_zc'];
    $mumu = 0;
...
if ($zc_auto == 0) {
    $result = $db1->query("SELECT kauser,dai_zc FROM web_member WHERE dai = '" . $username . "' AND zs=1 ORDER BY dai_zc DESC LIMIT 1");
    $numzc2 = $result->fetch_array();
    if ($numzc2 != "" && $minzc < $numzc2['dai_zc']) {
        $minzc = $numzc2['dai_zc'];
        $minzc_user = "会员账号:" . $numzc2['kauser'] . "代理占成" . $minzc . "%";
    }
}
$mumu = 0;
$result1 = $db1->query("select SUM(cs) As sum_m  From web_member Where zs=1 and   dai='" . $username . "' order by id desc");
$rsw = $result1->fetch_array();
if ($rsw[0] != "") {
    $mumu += $rsw[0];
}
...
if ($_GET['act'] == "edit") {
    ...
    if ($sf < $minzc) {
        echo "<script>alert('代理最低占成不能低于" . $minzc_user . "');window.history.go(-1);</script>";
        exit;
    }
    if ($zc_auto == 0) {
        if ($zc_all == 0) {
    ...
    if ($Current_KitheTable[29] != 0) {
        if ($_POST['sj'] != $zong_zc || $_POST['sf'] != $dai_zc) {
            echo "<script>alert('开盘期间不允许修改占成!'); window.history.go(-1);</script>";
            exit;
        }
    } else {
        if ($_POST['sj'] != $zong_zc || $_POST['sf'] != $dai_zc) {
            $str1 = "代理占成修改<br>修改前总代理占成:" . $zong_zc . "%,代理占成:" . $dai_zc . "%";
            $str2 = "代理占成修改<br>修改后总代理占成:" . $_POST['sj'] . "%,代理占成:" . $_POST['sf'] . "%";
            $sql = "INSERT INTO  web_user_log set adddate='" . $utime . "',adduser='" . $kauser . "',kauser='" . $username . "',lx=2,text1='" . $str1 . "',text2='" . $str2 . "',ip='" . $userip . "'";
            $db1->query($sql);
        }
    }
    $sql = "update  web_agent set uid='',xm='" . $xm . "',cs='" . $cs . "',ts='" . $ts . "',adddate='" . $utime . "',dai_zc='" . $sf . "',zong_zc='" . $sj . "',guan_zc='" . $guan_zc . "',dagu_zc='" . $dagu_zc . "',gs_zc='" . $gs_zc . "',pz='" . $pz . "',stat='" . $stat . "',ty='" . $ty . "',zc='" . $zc . "' where id='" . $userid . "'  order by id desc";
    if (!$db1->query($sql)) {
        exit("数据库修改出错");
    }
    if ($_POST['sj'] != $zong_zc || $_POST['sf'] != $dai_zc) {
        if ($zc_all == 0) {
            if ($zc_auto == 1) {
                $db1->query("update web_member set uid='',dai_zc=" . $sf . ",zong_zc=" . $sj . ",guan_zc=" . $guan_zc . ",dagu_zc=" . $dagu_zc . ",gs_zc=" . $gs_zc . "  where zs=1 and dai='" . $username . "'");
            } else {
                $db1->query("update web_member set uid='',dagu_zc=" . $dagu_zc . ",guan_zc=" . $guan_zc . ",zong_zc=" . $sj . ",gs_zc=100-dagu_zc-guan_zc-zong_zc-dai_zc  where zs=1 and dai='" . $username . "'");
            }
        } else {
            if ($zc_auto == 1) {
                $db1->query("update web_member set uid='',dai_zc=" . $sf . ",zong_zc=" . $sj . ",guan_zc=" . $guan_zc . ",dagu_zc=" . $dagu_zc . ",gs_zc=" . $gs_zc . "  where zs=1 and dai='" . $username . "'");
            } else {
                $db1->query("update web_member set uid='',gs_zc=" . $gs_zc . ",guan_zc=" . $guan_zc . ",zong_zc=" . $sj . ",dagu_zc=100-gs_zc-guan_zc-zong_zc-dai_zc  where zs=1 and dai='" . $username . "'");
            }
        }
    }
    del_online($username, 1);
    echo "<script>alert('修改成功!'); window.location.href = 'main.php?action=user_dai_edit&uid=" . $uid . "&vip=" . $vip . "&id=" . $userid . "';</script>";
    exit;
}?>
...
function SubChk() {
        ...
        if ($("cs").value == '') {
            $("cs").focus();
            alert("请输入总信用额度!!!"); return false;
        } if (isNaN($("cs").value)) {
            alert('总信用额度只能输入数字');
            $("cs").focus(""); return false;
        }
        if ($("cs").value > eval($("maxcs").value)) { $("cs").focus(); alert("信用额度不能大于最高信用额度!!!"); return false; } 
        if ($("cs").value < eval($("mincs").value)) {
            $("cs").focus(); alert("信用额度不能小于已用信用额度!!!");
            return false;
        }
        if (!confirm("是否确定修改代理?")) { return false; }
    }
    </script>
<script>
<? if ($zc_auto == 0) {?>
    ...
    function ChangeSelectorSF() {
        ...
        if (zzc > "<?=$iend?>") {
            ...
            $("sf").focus();
            alert("总代理占成数+代理占成数不能大于可用占成数<?=$iend?>%！"); return false;
        }
    } 
<?} else {?>
        function ChangeSelectorSJ() {
    ...
    <?}?>
            ...
            function Ckpzall(ckid) { if (ckid == 1){ $("gsmsg").innerHTML = "各级走飞剩余金额"; } else { $("gsmsg").innerHTML = ""; }}
</script>
<body style="min-width: 1200px;width: 100%">
<div id="tit" class="tit" >
      <div class="tit_left floatleft"><img src="/images/tit_01.png" width="5" height="31" alt=""></div>
      <div class="tit_center floatleft font_bold"><span class="font_g">&nbsp;代理管理－</span><span class="font_b">修改代理&nbsp;</span></div>
    ...
    <tbody><tr>
            <td height="20" colspan="2" align="center" class="t_list_caption">总代理(<?=$zong?>)资料简述
            </td>
        </tr>
        <tr>
    ...
    <?}?>
    <form action="main.php?action=user_dai_edit&uid=<?=$uid?>&vip=<?=$vip?>&act=edit&id=<?=$_GET['id']?>" method="post"
        name="form1" id="form1" onSubmit="return SubChk()">
       ...
       <table width="900" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
            ...
            <tr>
                ...
                <td height="30" colspan="3" bgcolor="#FFFFFF" class="t_edit_td">
                    <input name="pass" type="password" class="inp1"
                        id="pass" />(密码不修改请留空)&nbsp;&nbsp;密码必须至少6个字元长，最多12个字元长，并只能有数字(0-9)，及英文大小写字母
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td height="30" colspan="3" bgcolor="#FFFFFF" class="t_edit_td">
                    <select name="sf" id="sf" onchange=ChangeSelectorSF()>
                        <? $bb = $minzc;
                     for (; $bb <= $iend; $bb += 5) {?>
                        <option value="<?=$bb?>"  <? if ($dai_zc==$bb) {?> selected="selected" <?}?>>
