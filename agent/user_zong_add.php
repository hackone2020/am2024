...
if ($guan != "") {
    $result = $db1->query("select * from web_agent where kauser='" . $guan . "' and lx=3 " . $select_sql . " order by id LIMIT 1");
...
if ($_GET['act'] == "add") {
    ...
    $userid = $row['id'];
    $result = $db1->query("select id,ds_lb,ds_id,ds,yg,ygb,ygc,ygd,xx,xxx,ds_lock from  web_user_ds where lx=3 and username='" . $guan . "' order by id");
    while ($image = $result->fetch_array()) {
        ...
        $db1->query("INSERT INTO web_user_ds set username='{$username}',userid='{$userid}',ds_lb='" . $image['ds_lb'] . "',ds_id='" . $image['ds_id'] . "',ds='" . $image['ds'] . "',yg='" . $yg . "',ygb='" . $ygb . "',ygc='" . $ygc . "',ygd='" . $ygd . "',xx='" . $image['xx'] . "',xxx='" . $image['xxx'] . "',ds_lock='" . $image['ds_lock'] . "',lx='2',pz='0',pz_sum='0',zf_sum='500000',dagu='" . $dagu . "',guan='" . $guan . "',zong='',dai=''");
    }
    echo "<script>alert('添加成功!');window.location.href='main.php?action=user_zong_list&uid=" . $uid . "&vip=" . $vip . "';</script>";
    exit;
}?>
...
<script>function SubChk() {
        if ($("agent").value == '') {$("guan").focus();alert("请选择上级!!"); return false;}
        ...
        if (!confirm("是否确定添加总代理?")) { return false; }
        }</script>
               ...
               <? if ($zc_auto == 0) {?>
                    function ChangeSelectorSJ() {
                        ...
                        if (zzc > "<?=$iend?>") {
                            ...
                            $("sj").focus(); alert("股东占成数+总代理占成数不能大于可用占成数<?=$iend?>%！");
                            return false;
                        }
                    }
                    function ChangeSelectorSF() {
                        ...
                        if (zzc > "<?=$iend?>")
                        {
                            ...
                            $("sf").focus(); alert("股东占成数+总代理占成数不能大于可用占成数<?=$iend?>%！");
                            return false;
                        }
                    }
                    <?
...
<body style="min-width: 1200px;width: 100%">
 <div id="tit" class="tit" >
      ...
      <div class="tit_center floatleft font_bold"><span class="font_g">&nbsp;总代理管理－</span><span class="font_b">添加总代&nbsp; </span></div>
      <div class="tit_right floatleft"><img src="/images/tit_03.png" width="5" height="31" alt=""></div>
      <div class="biaoti_right floatright">
        <table border="0" align="right" cellpadding="0" cellspacing="0">
            <tbody>
              <tr>
                <td height="28" style="color:#fff;">添加总代&nbsp; 
                    <select id="guan" name="guan"
                        onChange="var jmpURL='main.php?action=user_zong_add&uid=<?=$uid?>&vip=<?=$vip?>&guan='+this.options[this.selectedIndex].value ; if(jmpURL!='') {window.location=jmpURL;} else {this.selectedIndex=0 ;}">
                        <option value="">请选择上级账号</option>
                        <? $result = $db1->query("select * from web_agent where lx=3 " . $select_sql . " order by id desc");
while ($image = $result->fetch_array()) {?>
                        <option value="<?=$image['kauser']?>" <?if ($guan==$image['kauser']) {?>selected="selected"<?}?>>
                            <?=$image['kauser']?>
                        </option>
                        <?}?>
                    </select>
                </font> -- <a href="main.php?action=user_zong_list&uid=<?=$uid?>&vip=<?=$vip?>">回上一页</a></td>
        </tr>
		</tbody>
    </table>
	</div></div>
    ...
    <br>
     <table width="900" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
        <tr>
           <td height="20" colspan="2" align="center" bgcolor="#FFFFFF" class="t_list_caption">股东(<?=$guan?>)资料
            </td>
        </tr>
        <tr>
    ...
    <form name="form1" onSubmit="return SubChk()" method="post"
        action="main.php?action=user_zong_add&uid=<?=$uid?>&vip=<?=$vip?>&act=add&guan=<?=$guan?>">
        ...
        <table width="900" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
            ...
            <tr>
                <td height="30" align="right" bgcolor="#FFFFFF" class="t_edit_caption">上级股东:</td>
                <td height="30" bgcolor="#FFFFFF" class="t_edit_td"><input name="agent" type="hidden" id="agent" value="<? if ($guan !="" ) {?><?=$guan?><?}?>"
                    />
                    <? if ($guan == "") {?>
                   请选择上级!!
                    <?} else {?>
            ...
            <tr>
                ...
                <td height="30" bgcolor="#FFFFFF" class="t_edit_td">
                    ...
                    <input type="button" onClick="check_user();" value="检查账号"
                        class="za_button" />&nbsp;帐号必须至少1个字元长，最多12个字，及英文小写字母
                </td>
            </tr>
            <tr>
                <td width="16%" height="30" align="right" bgcolor="#FFFFFF" class="t_edit_caption">密码:</td>
                <td height="30" bgcolor="#FFFFFF" class="t_edit_td">
                    <input name="pass" type="password" class="za_text" id="pass" />
            ...
            <tr>
                ...
                <td height="30" bgcolor="#FFFFFF" class="t_edit_td">
                    ...
                    <input name="pass2" type="password" class="za_text" id="pass2" />
                </td>
            </tr>
            <tr>
                <td height="30" align="right" bgcolor="#FFFFFF" class="t_edit_caption">名称:</td>
                <td height="30" bgcolor="#FFFFFF" class="t_edit_td">
                    <input name="xm" type="text" class="za_text" id="xm" />
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td height="30" bgcolor="#FFFFFF" class="t_edit_td">
                    <input name="cs" type="text" class="za_text" id="cs"
                        onkeyup="value=value.replace(/[^\d\.\/]/ig,'')" value="0" />最高信用额度:
                    <?=$maxcs?>
        ...
        default:
            ...
            <tr>
                ...
                <td height="30" border color="#CCCCCC" bgcolor="#FFFFFF" class="t_edit_td">
