...
if ($_GET['act'] == "edit") {
	...
	echo "<script>alert('设定成功!');window.location.href='main.php?action=user_mem_set&uid=" . $uid . "&vip=" . $vip . "&id=".$userid."';</script>";
	exit;
} 
?>
...
<form action="main.php?action=user_mem_set&uid=<?=$uid?>&vip=<?=$vip?>&act=edit&id=<?=$userid?>" method="post" name="form1" id="form1" onSubmit="return SubChk()">
...
?>
          <tr>
            ...
            <input type="submit" class="btn1"  name="Submit" value="确定" />            </td>
          </tr>
      </tbody>
  </table>
</form>
