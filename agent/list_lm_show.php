...
<?} else {
    ...
    while ($image = $result->fetch_array()) {?>
          <tr bgcolor="#FFFFFF">   
            ...
            <td height="28" align="center"><?=$image['sl']."*".$image['sz']."=".$image['zs']?>
       ...
       <? $strtmp = "look.html?action=list_look&uid=".$uid."&vip=".$vip."&kithe=".$kithe."&class2=".$class2."&class3=".$image['class3'];?>
            <button class="btn1" onmouseover="this.className='btn2';return true;" onMouseOut="this.className='btn1';return true;" onClick="ops('<?=$strtmp?>',400,750);"><?=round($image['sum_sum'], 2)?></button>    </td>    
        ...
        if ( $vip == 0 && $pz_of == 0 && $Current_KitheTable[29] != 0 && $Current_KitheTable[15] != 0 )
        {
            if ( $list_table_zf == 0 )
            {
                $zf_str = "<button class=\"btn1\"  onmouseover=\"this.className='btn2';return true;\" onMouseOut=\"this.className='btn1';return true;\" onclick=\"show_win_lm('".$image['class3']."',".$image['sz'].",'".$list_table_zf."','".$image['bl']."','".$image['bl2']."','".$ds_temp[$bl_dslb]['zf_ds']."','".$ids."','".$class2."');\" >手补</button>";
            }
            else
            {
                $zf_str = "<button class=\"btn1\"  onmouseover=\"this.className='btn2';return true;\" onMouseOut=\"this.className='btn1';return true;\" onclick=\"show_win_lm('".$image['class3']."',".$image['sz'].",'".$list_table_zf."','".$image['bl']."','".$image['bl2']."','".$ds_temp[$bl_dslb]['zf_ds']."','".$ids."','".$class2."');\" >手补 ".$list_table_zf."</button>";
            }
        }
        else
        {
      ...
      <tr bgcolor="#FFFFFF">    <td height="28" colspan="6" align="center"><?=get_page_html($pagecount, $page)?>
    </td>  </tr><?}?>
</table>&nbsp;