      ...
      <div class="biaoti_right floatright">
       <table width="874" border="0" cellspacing="0" cellpadding="0">
        ...
        <td height="28" style="color:#fff;">
            ...
            <?}?>
            &nbsp;&nbsp;(以下所有结果均包含退水)&nbsp;&nbsp;大股东:
            <?=$user?> --<a href="javascript:history.go( -1 );">回上一页</a> -- <a href="main.php?action=report&uid=<?=$uid?>&vip=<?=$vip?>">回报表查询</a>
        </td>
        </tr>
        </tbody>
    </table></div></div>
    <? $result = $db1->query("SELECT guan,sum(sz) as zs,sum(sum_m) as sum_m,sum(user_sf) usersf,sum(user_sq) user_ds,sum(dai_sq) dai_ds,sum(zong_sq) zong_ds,sum(guan_sq) guan_ds,sum(dagu_sq) dagu_ds,sum(daguzc) as daguzc,sum(dagu_sf) as dagusf,sum(gszc) as gszc,sum(gs_sf) as gssf FROM " . $tb_name . "  WHERE visible=1 and dagu='" . $user . "' and dai!='" . $user . "' and username!='" . $user . "' " . $vvv . " group by guan order by guan DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
        <tr>
            <td height="18" colspan="11" align="center" nowrap="nowrap" class="t_list_caption">下级股东注单</td>
        </tr>
        <tr>
            <td height="18" align="center" nowrap="nowrap" class="t_list_tr_3">序号</td>
            <td align="center" class="t_list_tr_3">股东</td>
            ...
            <td align="center" class="t_list_tr_3">大股东占成金额</td>
            <td align="center" class="t_list_tr_3">大股东赚水</td>
            <td align="center" bgcolor="#CC0000" class="t_list_tr_3">大股东结果</td>
            <td align="center" class="t_list_tr_3">与股东交收</td>
            <td align="center" class="t_list_tr_3">与公司交收</td>
        </tr>
        ...
        <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
            ...
            <td align="center" nowrap="nowrap"><a href="main.php?action=report_m3&uid=<?=$uid?>&vip=<?=$vip?>&kithe=<?=$kithe?>&user=<?=$rs['guan']?>
        &date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>"><?=round($sum_m, 2)?>
                </a> </td>
            <td height="28" align="center" nowrap="nowrap">
                <?=round($usersf, 2)?>
            </td>
        ...
        <tr class="t_list_tr_2">
            ...
            <td align="center" nowrap="nowrap">
                <?=get_sf(round($z_dagusf1 + $z_gssf1, 2))?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=get_sf(round(0 - $z_gssf1, 2))?>
            </td>
        </tr>
    </table><br>
    <? }?>
    <? $result = $db1->query("SELECT username,sum(sz) as zs,sum(sum_m) as sum_m,sum(user_sf) usersf,sum(user_sq) user_ds,sum(dai_sq) dai_ds,sum(daguzc) as daguzc,sum(dagu_sf) as dagusf,sum(gszc) as gszc,sum(gs_sf) as gssf FROM " . $tb_name . "  WHERE visible=1 and dagu='" . $user . "' and dai='" . $user . "' and username!='" . $user . "' " . $vvv . " group by username order by username DESC");
$ii = 0;
if ($result->num_rows!= 0) {?>
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
        <tr>
        ...
        <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
            ...
            <td align="center" nowrap="nowrap"><a href="main.php?action=report_m6&uid=<?=$uid?>&vip=<?=$vip?>&kithe=<?=$kithe?>&user=<?=$rs['username']?>&date_start=<?=$date_start?>
                    &date_end=<?=$date_end?>&gtype=<?=$gtype?>"><?=round($sum_m, 2)?>
                </a></td>
            <td height="28" align="center" nowrap="nowrap">
                <?=round($usersf, 2)?>
            </td>
        ...
        <tr class="t_list_tr_2">
            ...
            <td align="center" nowrap="nowrap">
                <?=get_sf(round($z_dagusf2 + $z_gssf2, 2))?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=get_sf(round(0 - $z_gssf2, 2))?>
            </td>
        </tr>
    </table><br>
    <?}?>
    <? $result = $db1->query("SELECT username,sum(sz) as zs,sum(sum_m) as sum_m,sum(user_sf) usersf,sum(user_sq) user_ds,sum(gszc) as gszc,sum(gs_sf) as gssf FROM " . $tb_name . "  WHERE visible=1 and lx=4 and username='" . $user . "' " . $vvv . " group by username order by username DESC");
$ii = 0; 
?>
        <? if ($result->num_rows != 0) {?>
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
        <tr>
            <td height="18" colspan="7" nowrap="nowrap" class="t_list_caption">大股东补货走飞结果</td>
        </tr>
        <tr>
            <td height="18" align="center" nowrap="nowrap" class="t_list_tr_3">序号</td>
        ...
        <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
            ...
            <td align="center" nowrap="nowrap">
                <a href="main.php?action=report_m6&uid=<?=$uid?>&vip=<?=$vip?>&kithe=<?=$kithe?>&user=<?=$rs['username']?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&lx=4"><?=round($sum_m, 2)?></a>
            </td>
            <td height="28" align="center" nowrap="nowrap">
                <?=round($usersf, 2)?>
            </td>
   ...
   <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
        ...
        <tr class="t_list_tr_2">
            ...
            <td align="center" nowrap="nowrap">
                <?=round($z_dagu1 + $z_dagu2 - $z_sum3, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=round($z_daguds1 + $z_daguds2, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=round($z_dagusf1 + $z_dagusf2 + $z_dagusf3, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=get_sf(round($z_dagusf1 + $z_gssf1, 2) + round($z_dagusf2 + $z_gssf2, 2))?>
            </td>
            <td align="center" nowrap="nowrap">
                <?=get_sf(round(0 - $z_gssf1, 2) + round(0 - $z_gssf2, 2) + round(0 - $z_gssf3, 2))?>
            </td>
        </tr>
    </table>
    <?} else {?>
