...
<body bgcolor="#FFFFFF" text="#000000" leftmargin="0" topmargin="0" vlink="#0000FF" alink="#0000FF">
    <table width="900" height="24" border="0" cellpadding="0" cellspacing="0">
        ...
        <td width="775" class="m_tline"><font color="#CC0000">当前查询</font>--<?if ($kithe != "") {?>第<?=$kithe?>期<?} else {?>日期区间：<?=$date_start?> ~<?=$date_end?><?} ?>
            &nbsp;&nbsp;类型:<?if ($class2 != "") {?><?=$class2?><?} else {?>全部<?}?><?if ($user != "") {?>&nbsp;&nbsp;会员:<?=$user?><?}?>
           --<a href="javascript:history.go( -1 );">回上一页</a> -- <a href="main.php?action=report&uid=<?=$uid?>&vip=<?=$vip?>">回报表查询</a>
        </td>
...
if ($result->num_rows != 0) {?>
    <table width="900" border="0" cellpadding="0" cellspacing="1" bgcolor="#000000">
        <tr>
            <td width="154" height="20" align="center" class="m_title_reall">单号/时间</td>
            <td width="74" class="m_title_reall">期数/盘口</td>
            <td width="75" align="center" class="m_title_reall">用户</td>
            <td width="206" align="center" nowrap="nowrap" class="m_title_reall">下注内容</td>
            <td width="76" align="center" nowrap="nowrap" class="m_title_reall">赔率</td>
            <td width="88" align="center" nowrap="nowrap" class="m_title_reall">下注金额</td>
            <td width="71" align="center" class="m_title_reall">退水金额</td>
            <td width="97" align="center" class="m_title_reall">会员结果</td>
        </tr>
...
<a href="main.php?action=report_m6_gs&uid=<?=$uid?>&vip=<?=$vip?>&kithe=<?=$kithe?>&user=<?=$user?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>?gtype=<?=$gtype?>&lx=<?=$lx?>&page=1">首页</a>
<a href="main.php?action=report_m6_gs&uid=<?=$uid?>&vip=<?=$vip?>&kithe=<?=$kithe?>&user=<?=$user?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&lx=<?=$lx?>&page=<?=$pre?>">上一页</a>
<a href="main.php?action=report_m6_gs&uid=<?=$uid?>&vip=<?=$vip?>&kithe=<?=$kithe?>&user=<?=$user?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&lx=<?=$lx?>&page=<?=$next?>">下一页</a>
<a href="main.php?action=report_m6_gs&uid=<?=$uid?>&vip=<?=$vip?>&kithe=<?=$kithe?>&user=<?=$user?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&lx=<?=$lx?>?page=<?=$last?>">末页</a>
    ...
    <table width="900" border="0" cellspacing="1" cellpadding="0" class="m_tab" bgcolor="#000000">
        <tr class="m_title_2">
            <td width="101">本次查询总计</td>
            <td width="122">组数</td>
            <td width="218">下注金额</td>
            <td width="124">结果</td>
        </tr>
        <tr class="m_rig">
            <td>分
