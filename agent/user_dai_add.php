...
if ($zong != "") {
    ...
    $guan = $row['guan'];
    $pz_of = 0;
    $result = $db1->query("select count(*) from web_agent  where (kauser='" . $zong . "' or kauser='" . $guan . "' or kauser='" . $dagu . "') and pz=1 order by id LIMIT 1");
...
if ($_GET['act'] == "add") {
    ...
    $sql = "INSERT INTO  web_agent set uid='',kauser='{$username}',kapassword='{$pass}',xm='{$xm}',slogin='{$utime}',sip='{$userip}',zlogin='{$utime}',zip='{$userip}',adduser='{$kauser}',look='0',lx='1',dagu='{$dagu}',guan='{$guan}',zong='{$zong}',cs='{$cs}',ts='{$cs}',adddate='{$utime}',stat='{$stat}',ty='{$ty}',pz='{$pz}',pzall='{$pzall}',abcd='',zc='{$zc}',zc_all='{$zc_all}',zc_auto='{$zc_auto}',report='{$report}',gs_zc='{$gs_zc}',dagu_zc='{$dagu_zc}',guan_zc='{$guan_zc}',zong_zc='{$sj}',dai_zc='{$sf}'";
    if (!$db1->query($sql)) {
        exit("数据库修改出错");
    }
    ...
    $userid = $row['id'];
    $result = $db1->query("select id,ds_lb,ds_id,ds,yg,ygb,ygc,ygd,xx,xxx,ds_lock from  web_user_ds where lx=2 and username='" . $zong . "' order by id");
    while ($image = $result->fetch_array()) {
        ...
        $db1->query("INSERT INTO web_user_ds set username='{$username}',userid='{$userid}',ds_lb='" . $image['ds_lb'] . "',ds_id='" . $image['ds_id'] . "',ds='" . $image['ds'] . "',yg='" . $yg . "',ygb='" . $ygb . "',ygc='" . $ygc . "',ygd='" . $ygd . "',xx='" . $image['xx'] . "',xxx='" . $image['xxx'] . "',ds_lock='" . $image['ds_lock'] . "',lx='1',pz='0',pz_sum='0',zf_sum='500000',dagu='" . $dagu . "',guan='" . $guan . "',zong='" . $zong . "',dai=''");
    }
    echo "<script>alert('添加成功!');window.location.href='main.php?action=user_dai_list&uid=" . $uid . "&vip=" . $vip . "';</script>";
    exit;
}?>
...
<script>
function SubChk() {
        if ($("agent").value == '') { $("zong").focus(); alert("请选择上级!!"); return false; }
        ...
        if (!confirm("是否确定添加代理 ? ")) { return false;  }
    }
    </script>
 ...
 <div id="tit" class="tit" >
      ...
      <div class="biaoti_right floatright">
        <table border="0" align="right" cellpadding="0" cellspacing="0">
            <tbody>
              <tr>
                <td height="28" style="color:#fff;">添加代理&nbsp;
                    <select id="zong" name="zong" onChange="var jmpURL='main.php?action=user_dai_add&uid=<?=$uid?>&vip==<?=$vip?>&zong='+this.options[this.selectedIndex].value; if(jmpURL!=''){window.location=jmpURL;} else {this.selectedIndex=0 ;}">
                        <option value="">请选择上级账号</option>
                        <?$result = $db1->query("select * from web_agent where lx=2 " . $select_sql . " order by id desc");
while ($image = $result->fetch_array()) {
  ?>
  <option value="<?=$image['kauser']?>" <?if ($zong == $image['kauser']) {?> selected="selected" <?} ?>><?=$image['kauser']?></option>      <?}?>
                    </select>
                </font> -- <a href="main.php?action=user_dai_list&uid=<?=$uid?>&vip=<?=$vip?>">回上一页</a></td>
                </tr>
                </tbody>
                </table>
    </div></div>
    <?if ($zong != "") {?>
     <table width="900" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
        <tr>
            <td height="20" colspan="2" align="center" bgcolor="#FFFFFF" class="t_list_caption">总代理(<?=$zong?>)资料
            </td>
        </tr>
        <tr>
            <td width="16%" height="20" align="right" class="t_edit_caption">下注:</td>
            <td height="30" bgcolor="#FFFFFF" class="t_edit_td">可用占成数:
                <?=$zongmaxzc?>
                %&nbsp;&nbsp;&nbsp;总信用额:
                <?=$zongmaxcs?>
                &nbsp;&nbsp;&nbsp;已用信用额:
    ...
    <?}?>
    <form name="form1" onSubmit="return SubChk()" method="post" action="main.php?action=user_dai_add&uid=<?=$uid?>&vip=<?=$vip?>&act=add&zong=<?=$zong?>"> 
          <table width="900" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
            <tr>
            ...
            <tr>
               ...
               <input name="agent" type="hidden" id="agent" value="<? if ($zong !="" ) {?><?=$zong?>"><?}if ($zong == "") {?>请选择上级!!<?} else {?><?=$zong?><?}?></td>
            </tr>        
            <tr>
                ...
                <td height="30" bgcolor="#FFFFFF" class="t_edit_td">
                    ...
                    <input type="button" onClick="check_user();" value="检查账号" class="za_button" />&nbsp;帐号必须至少1个字元长，最多12个字,并只能有数字(0-9)，及英文大小写字母</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td bgcolor="#FFFFFF" class="t_edit_td"><input name="pass2" type="password" class="za_text" id="pass2" />
                </td>
            </tr>
            <tr>
                <td width="16%" height="30" align="right" bgcolor="#FFFFFF" class="t_edit_caption">名称:</td>
                <td bgcolor="#FFFFFF" class="t_edit_td"><input name="xm" type="text" class="za_text" id="xm" /></td>
            </tr>
            <tr>
            ...
            <tr>
               <td width="16%" height="30" align="right" bgcolor="#FFFFFF" class="t_edit_caption">总信用额:</td>
                <td bgcolor="#FFFFFF" class="t_edit_td"><input name="cs" type="text" class="za_text" id="cs"
                        onkeyup="value=value.replace(/[^\d\.\/]/ig,'')" value="0" />

                    最高信用额度:
                    <?=$maxcs?><input name="maxcs" type="hidden" id="maxcs" value="<?=$maxcs?>" />
                </td>
            </tr> 
            <?
            if ($pz_of == 0) {
            ...
            <tr>
                ...
                <td height="30" bgcolor="#FFFFFF" class="t_edit_td">
                    <input type="radio" name="rpz" id="rpz" value="0" checked="checked" />允许<input type="radio"
                        name="rpz" id="rpz" value="1" onClick="StopPZ()" />禁止
                </td>
            </tr>
<?}?>
            <tr>
...
<?}?>
            ...
            <tr>
                <td width="16%" height="30" align="right" bgcolor="#FFFFFF" class="t_edit_caption">代理占成数:</td>
                <td height="30" bordercolor="#CCCCCC" bgcolor="#FFFFFF" class="t_edit_td">
...
<?}?>
            ...
            <tr>
                <td width="16%" height="30" align="right" bgcolor="#FFFFFF" class="t_edit_caption">退水设定:</td>
                <td height="30" bordercolor="#CCCCCC" bgcolor="#FFFFFF" class="t_edit_td">
            ...
            <tr>
                <td height="30" colspan="2" align="center" bgcolor="#FFFFFF" class=""><input type="submit"
                       class="btn1"  name="button" id="button" value="确定" />
                </td>
            </tr>
        </table>
    </form><iframe name="check_frame" src="" width="0" height="0" style="display:none"></iframe>
</body>