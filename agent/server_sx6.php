...
$result = $db1->query("SELECT count(*) FROM (SELECT DISTINCT(class3) FROM " . $tb_name . " WHERE " . $select_sql . " and qx=0 " . $vvv . " and class1='生肖' and class2='六肖') AS temp_tan");
...
<?} else {
    ...
    while ($image = $result->fetch_array()) {?>
        ...
        <? $strtmp = "look.html?action=list_look&uid=" . $uid . "&vip=" . $vip . "&kithe=" . $kithe . "&class2=六肖&class3=" . $image['class3'];?>
        ...
        <?if ($image['sum_wai'] != 0 && $image['sum_wai'] != "") { $strtmp = "look.html?action=list_look&uid=" . $uid . "&vip=" . $vip . "&kithe=" . $kithe . "&lx=" . $lx . "&class2=六肖&class3=" . $image['class3']?>
             <center><button class=btn1 onmouseover=this.className='btn2';return true; onMouseOut=this.className='btn1';return true; onClick=ops('<?=$strtmp?>',400,750) >
