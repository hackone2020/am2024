...
if ($_GET['act'] == "edit") {
    ...
    if ($zc_auto == 0) {
        ...
        if ($zc_all == 0) {
            switch ($lx) {
                case "4":
                    $sql = "update web_member  set uid='',xm='" . $xm . "',adddate='" . $utime . "',stat='" . $stat . "',ty='" . $ty . "',xy='" . $xy . "',abcd='" . $abcd . "',dagu_zc='" . $dai_zc . "',gs_zc=100-dagu_zc-guan_zc-zong_zc-dai_zc  where kauser='" . $username . "'";
                    break;
                case "3":
                    $sql = "update web_member  set uid='',xm='" . $xm . "',adddate='" . $utime . "',stat='" . $stat . "',ty='" . $ty . "',xy='" . $xy . "',abcd='" . $abcd . "',guan_zc='" . $dai_zc . "',gs_zc=100-dagu_zc-guan_zc-zong_zc-dai_zc  where kauser='" . $username . "'";
                    break;
                case "2":
                    $sql = "update web_member  set uid='',xm='" . $xm . "',adddate='" . $utime . "',stat='" . $stat . "',ty='" . $ty . "',xy='" . $xy . "',abcd='" . $abcd . "',zong_zc='" . $dai_zc . "',gs_zc=100-dagu_zc-guan_zc-zong_zc-dai_zc  where kauser='" . $username . "'";
                    break;
                default:
                    break;
            }
            ...
            switch ($lx) {
                case "4":
                    $sql = "update web_member  set uid='',xm='" . $xm . "',adddate='" . $utime . "',stat='" . $stat . "',ty='" . $ty . "',xy='" . $xy . "',abcd='" . $abcd . "',dagu_zc=100-gs_zc-guan_zc-zong_zc-dai_zc  where kauser='" . $username . "'";
                    break;
                case "3":
                    $sql = "update web_member  set uid='',xm='" . $xm . "',adddate='" . $utime . "',stat='" . $stat . "',ty='" . $ty . "',xy='" . $xy . "',abcd='" . $abcd . "',guan_zc='" . $dai_zc . "',dagu_zc=100-gs_zc-guan_zc-zong_zc-dai_zc  where kauser='" . $username . "'";
                    break;
                case "2":
                    $sql = "update web_member  set uid='',xm='" . $xm . "',adddate='" . $utime . "',stat='" . $stat . "',ty='" . $ty . "',xy='" . $xy . "',abcd='" . $abcd . "',zong_zc='" . $dai_zc . "',dagu_zc=100-gs_zc-guan_zc-zong_zc-dai_zc  where kauser='" . $username . "'";
                    break;
                default:
                    break;
            }
    ...
    echo "<script>alert('修改成功!');window.location.href='main.php?action=user_mem_zs_list&uid=" . $uid . "&vip=" . $vip . "';</script>";
    exit;
}
?>
...
<div id="tit" class="tit" >
      ...
      <div class="tit_center floatleft font_bold"><span class="font_g">修改直属会员&nbsp;</span><span class="font_b">修改直属会员&nbsp;</span></div>
      <div class="tit_right floatleft"><img src="/images/tit_03.png" width="5" height="31" alt=""></div>
      <div class="biaoti_right floatright">
          <a href="main.php?action=user_mem_zs_list&uid=<?=$uid?>&vip=<?=$vip?>" >回上一页</a>
      </div>
</div>  
<?if ($dai != "") {?>
<br>
 <table width="900" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
    <tbody><tr>
       <td height="20" colspan="2" align="center" class="t_list_caption">上级(<?=$dai?>)资料简述</td>
    </tr>
    <tr>
        <td width="16%" height="20" align="right" class="t_edit_caption">下注:</td>
        <td class="t_edit_td">可用占成数:<?=$daimaxzc?>%&nbsp;&nbsp;&nbsp;总信用额:<?=$daimaxcs?>&nbsp;&nbsp;&nbsp;已用信用额:<?=$mumu?>&nbsp;&nbsp;&nbsp;
        剩余信用额:<?=$daimaxcs -$mumu?>
        </td>
    </tr>
</table>
<?}?>
<form name="form1" onSubmit="return SubChk()" method="post" action="main.php?action=user_mem_zs_edit&uid=<?=$uid?>&vip=<?=$vip?>&act=edit&id=<?=$userid?>">
<table width="900" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
            <tr>
    ...
    <tr>
        ...
        <td height="30" colspan="3" bgcolor="#FFFFFF" class="t_edit_td" ><?=$dai?>&nbsp;&nbsp;&nbsp;&nbsp;直属类型:<?=$zslabel?></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td height="30" colspan="3" bgcolor="#FFFFFF" class="t_edit_td" >
            <input name="pass" type="password" class="inp1"  id="pass" />(密码不修改请留空)&nbsp;&nbsp;密码必须至少8个字元长，最多12个字元长，并只能有数字(0-9)，及英文大小写
        </td>
    </tr>
    <tr>
         <td width="16%" height="30" align="right" bgcolor="#FFFFFF" class="t_edit_caption">确认密码:</td>
        <td height="30" colspan="3" bgcolor="#FFFFFF" class="t_edit_td" >
            <input name="pass2" type="password" class="inp1" id="pass2" /></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td height="30" colspan="3" bgcolor="#FFFFFF" class="t_edit_td" >
            <input name="cs" type="text" class="inp1" id="cs"  onkeyup="value=value.replace(/[^\d\.\/]/ig,'')" value="<?=$cs?>" /><?=$maxcs?>
    ...
    <tr>
        ...
        <td height="30" colspan="3" bgcolor="#FFFFFF" class="t_edit_td" ><?if ($zc_auto == 0) {?>
                <select class="za_select_02" name="sj"  id="zc">
                <?
                    $bb = $istar;
                    for (; $bb <= $iend; $bb += 5) {?>
                    <option value="<?=$bb?>" <? if ($bb == $dai_zc) {?>selected="selected"<?}?>>
                    <?
                        switch ($bb) {
                            case 0:
                                print "不占成";
    ...
    <tr>
        ...
        <td width="33%" height="17" bgcolor="#FFFFFF" ><?=$row['zlogin']?></td>
        <td width="17%" height="17" align="right"  bgcolor="#FFFFFF">最后登录IP:</td>
        <td width="34%" height="17" bgcolor="#FFFFFF" ><?=$row['zip']?></td>
    </tr>
    <tr>
        <td width="16%" height="17" align="right"  bgcolor="#FFFFFF">基本资料更新时间:</td>
        <td width="33%" height="17" bgcolor="#FFFFFF" ><?=$row['adddate']?></td>
        <td width="17%" height="17" align="right"  bgcolor="#FFFFFF">登录次数:</td>
        <td width="34%" height="17" bgcolor="#FFFFFF" ><?=$row['look']?>次</td>
    </tr>
    <tr>
        <td width="16%" height="17" align="right"  bgcolor="#FFFFFF">新增时间:</td>
        <td width="33%" height="17" bgcolor="#FFFFFF" ><?=$row['slogin']?></td>
        <td width="17%" height="17" align="right"  bgcolor="#FFFFFF">账号添加人:</td>
        <td width="34%" height="17" bgcolor="#FFFFFF" ><?=$row['adduser']?></td>
    </tr>
<?}?><tr>
