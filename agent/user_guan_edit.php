...
if ($_GET['act'] == "edit") {
    ...
    echo "<script>alert('修改成功!');window.location.href='main.php?action=user_guan_edit&uid=" . $uid . "&vip=" . $vip . "&id=" . $userid . "';</script>";
    exit;
}?>
...
<?if ($zc_auto == 0 && $zc_all == 0) {?>
        function ChangeSelectorSJ() {
            ...
            if (zzc > "<?=$iend?>") {
                ...
                $("sj").focus(); alert("大股东占成数+股东占成数不能大于可用占成数<?=$iend?>%！"); return false;
            }
        }
   ...
   <? } else {?>
        ...
        <?}?>
            ...
            { if (ckid == 1) { $("gsmsg").innerHTML = "各级走飞剩余金额"; } else { $("gsmsg").innerHTML = ""; } }
...
<div id="tit" class="tit" >
      ...
      <div class="biaoti_right floatright">
          <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>">回上一页</a>
      </div>
</div>  
    <?if ($dagu != "") {?>
     <table width="900" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
    <tbody><tr>
            <td height="20" colspan="2" align="center" class="t_list_caption">大股东(<?=$dagu?>)资料简述
            </td>
        </tr>
        <tr>
...
<input name="maxcs" type="hidden" id="maxcs" value="<?=$maxcs?>" />&nbsp;&nbsp;(已用信用额度:
        ...
        checked="checked" <?} ?> />允许<input type="radio" name="rpz" id="rpz" value="1" <?if ($pz == 1) {?> checked="checked" <?}?> onClick="StopPZ()" />禁止
                </td>
            </tr><?}?>
            <tr>
                 <td width="16%" height="30" align="right" bgcolor="#FFFFFF" class="t_edit_caption">投注:</td>
                <td height="30" colspan="3" bgcolor="#FFFFFF" class="t_edit_td">
                    <input type="radio" name="rty" id="rty" value="0" <?if ($ty == 0) {?> checked="checked" <?}?>/>开放投注<input type="radio" name="rty" id="rty" value="1" 
<? if ($ty == 1) {
?>  checked="checked"
<?}?>
onClick="StopTY()" />暂停投注
