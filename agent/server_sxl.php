...
$result = $db1->query("SELECT ds_lb,class1,class2 FROM `web_bl` WHERE class1='生肖连' group by class1,class2 order by id ASC");
$ii = 1;
$list_table = array();
$z_zs = $z_sum = $z_zfsum = 0;
while ($rs = $result->fetch_assoc()) {
    $result5 = $db1->query("SELECT sum(if(username='" . $kauser . "',sum_m,0)) as sum_wai,sum(if(username='" . $kauser . "',-sum_m," . $tb_zd . ")) as sum_sum,sum(sz) as zs,sum(if(username='" . $kauser . "',-sum_m*rate," . $tb_zd . "*rate)) as win FROM " . $tb_name . " WHERE " . $select_sql . " and qx=0 " . $vvv . " and class1='生肖连' and class2='" . $rs['class2'] . "'");
