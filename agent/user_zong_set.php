...
$result = $db1->query("select * from web_agent where id=" . $_GET['id'] . " and lx=2 ".$select_sql." order by id");
...
if ($_GET['act'] == "edit") {
	...
	echo "<script>alert('设定成功!');window.location.href='main.php?action=user_zong_set&uid=" . $uid . "&vip=" . $vip . "&id=".$userid."';</script>";
	exit;
} 
?>
...
<body style="min-width: 1200px;width: 100%">
     <div id="tit" class="tit" >
      ...
      <div class="tit_center floatleft font_bold"><span class="font_g">&nbsp;总代理详细设定</span></div>
      <div class="tit_right floatleft"><img src="/images/tit_03.png" width="5" height="31" alt=""></div>
      <div class="biaoti_right floatright">
    <table border="0" align="right" cellpadding="0" cellspacing="0">
		<tbody><tr>
            <td height="28" style="color:#fff;">&nbsp;账号:<?=$username?> -- 名称:<?=$xm?> -- <a href="main.php?action=user_zong_list&uid=<?=$uid?>&vip=<?=$vip?>" >回上一页</a></td>
          </tr>
          </tbody>
        </table>
</div></div>
<form action="main.php?action=user_zong_set&uid=<?=$uid?>&vip=<?=$vip?>&act=edit&id=<?=$userid?>" method="post" name="form1" id="form1" onSubmit="return SubChk()">
...
<tr bgcolor="#edf2f7">
    <td width="100" height="26" align="right">
        <?=$image['ds']?>:<input name="ds[]" type="hidden" id="ds[]" value="<?=$image['ds']?>" /><input name="dsid[]" type="hidden" id="dsid[]" value="<?=$image['ds_id']?>" />
    </td>
   <td align="center">
