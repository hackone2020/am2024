...
if ($_GET['pz'] == "0") {
    ...
    $kauser = $_GET['name'];
    $db1->query("update web_agent set uid='' where guan='{$kauser}'" . $select_sql);
    $db1->query("update web_agent_zi set uid='' where guan='{$kauser}'");
    $db1->query("update web_member set uid='' where guan='{$kauser}'" . $select_sql);
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为开启走飞！!'); window.location.href = 'main.php?action=user_guan_list&uid={$uid}&vip={$vip}';</script>";
    exit;
}
if ($_GET['pz'] == "1") {
    ...
    $db1->query("update web_agent_zi set uid='' where guan='{$kauser}'");
    $db1->query("update web_member set uid='' where guan='{$kauser}'" . $select_sql);
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为禁止走飞!\\n该账号及其下属代理商将关闭走飞服务!!!'); window.location.href = 'main.php?action=user_guan_list&uid={$uid}&vip={$vip}';</script>";
    exit;
}
if ($_GET['stat'] == "0") {
    ...
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为启用!'); window.location.href = 'main.php?action=user_guan_list&uid={$uid}&vip={$vip}&enable=y';</script>";
    exit;
}
if ($_GET['stat'] == "1") {
    ...
    $db1->query("update web_agent_zi set uid='' where guan='{$kauser}'");
    $db1->query("update web_member set uid='' where guan='{$kauser}'" . $select_sql);
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为停用!'); window.location.href = 'main.php?action=user_guan_list&uid={$uid}&vip={$vip}&enable=n';</script>";
    exit;
}
if ($_GET['ty'] == "0") {
    ...
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为开放投注!'); window.location.href = 'main.php?action=user_guan_list&uid={$uid}&vip={$vip}&enable=all';</script>";
    exit;
}
if ($_GET['ty'] == "1") {
    ...
    echo "<script>alert('设置用户[<" . $_GET['name'] . ">]为暂停投注!'); window.location.href = 'main.php?action=user_guan_list&uid={$uid}&vip={$vip}&enable=s';</script>";
    exit;
}
$sort = $_GET['sort'];
...
<body style="min-width: 1200px;width: 100%">
    ...
    <table border="0" align="right" cellpadding="0" cellspacing="0">
         <tbody>
            <tr>
                <td height="28" style="color:#fff;">股东管理:&nbsp;&nbsp;
...
echo "<option value='" . ($i + 1) . "'>" . ($i + 1) . "</option>";
            ...
            <input type="button" name="btn_search" value="快速查询" onClick="showSearchDlg();"
                    class="za_button">
            --
            <input type="button" name="append" value="新增" onClick="javascript:location.href='main.php?action=user_guan_add&uid=<?=$uid?>&vip=<?=$vip?>'" class="btn2" />
...
if ($cou == 0) {?>
    ...
    <tr bgColor="#ffffff">
        <td width="73" height="25" align="center" nowrap> <a href="main.php?action=user_zong_list&uid=<?=$uid?>&vip=<?=$vip?>&guan=<?=$image['kauser']?>"><?=$image['kauser']?></a>
                </td>
                <td width="73" height="25" align="center">
                    <?=$image['xm']?>
                </td>
                <td width="135" height="25" align="right"><?=$image['cs']?>/<font color=ff0000><?=$image['cs'] - $mumu?></font>
                </td>
        ...
        </td><td width="278" height="25" align="center" nowrap>
		<a href="main.php?action=user_guan_edit&uid=<?=$uid?>&vip=<?=$vip?>&id=<?=$image['id']?>">修改资料</a> <a href="main.php?action=user_guan_set&uid=<?=$uid?>&vip=<?=$vip?>&id=<?=$image['id']?>">详细设置</a> 
		<?
        if ($image['stat'] == 0) {
         ?>
         <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>&stat=1&name=<?=$image['kauser']?>&id=<?=$image['id']?>">停用</a>
         <?
        } else {
            ?>
       <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>&stat=0&name=<?=$image['kauser']?>&id=<?=$image['id']?>">
 <SPAN STYLE="color: rgb(255,0,0);">启用</SPAN></a>
       <? }?> / 
       <?
        if ($image['ty'] == 0) {
            ?>
        <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>&ty=1&name=<?=$image['kauser']?>&id=<?=$image['id']?>">暂停投注</a>
        <?} else {?>
           <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>&ty=0&name=<?=$image['kauser']?>&id=<?=$image['id']?>">
       ...
       <? }
                 ...
                 / 
                    <? if ($image['pz'] == 0) {?>
                    <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>&pz=1&name=<?=$image['kauser']?>&id=<?=$image['id']?>">禁止走飞
                    </a>
                    <? } else {?>
                    <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>&pz=0&name=<?=$image['kauser']?>&id=<?=$image['id']?>">
                        <SPAN STYLE="color: rgb(255,0,0);">允许走飞</SPAN>
                    </a>
                    <?
                    }
            ...
            <?}?>
            <tr>
                <td height="25" colspan="10" bgcolor="#FFFFFF">
                    <table width="98%" border="0" align="center" cellpadding="2" cellspacing="0"
                        ...
                        <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=1">首页</a>
                        <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=<?=$pre?>">上一页
                                </a> 
                        <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=<?=$next?>">下一页
                                </a>
                        <a href="main.php?action=user_guan_list&uid=<?=$uid?>&vip=<?=$vip?>&dagu=<?=$dagu?>&enable=<?=$enable?>&sort=<?=$vvv?>&orderby=<?=$vvv2?>&page=<?=$last?>">末页</a>
                            </td>
                        </tr>
                    </table>
                </td>
