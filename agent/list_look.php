...
$result = $db1->query("select count(*) from " . $tb_name . " where " . $select_sql . " " . $vvv . " order by adddate desc");
...
<td width="616" class="m_tline">&nbsp;
<?
if ($class1 != "过关") {
    echo $class1."&nbsp;".$class2."&nbsp;".$class3;
} else {
   echo  $class1;
}?>
  －<input name="button" type="button" class="za_button" onClick="javascript:location.reload();" value="更新" /> </td>      
        ...
        <tr>
            <td>
                ...
                <tr> 
                    ...
                    <td width="61" align="center" nowrap="nowrap" class="m_title_ft">您占成</td>
...
<?}?>
      ...
      <?
$result = $db1->query("select * from " . $tb_name . " where " . $select_sql . " " . $vvv . " order by adddate desc limit " . $offset . "," . $pagesize);
if ($result->num_rows == 0) {?>
    <tr><td align="center" bordercolor="cccccc" bgcolor="#FFFFFF" colspan="100%">暂无下注记录！</td></tr>";
<?} else {
    while ($image = $result->fetch_array()) {?>
        ...
        if ($image['class1'] == "连码" || $image['class1'] == "自选不中") {
          ...
          <td align="center" nowrap="nowrap" bordercolor="cccccc" bgcolor="#FFFFFF"><?=$image[$vv . "zc"]?></td>        
          <td align="center" nowrap="nowrap" bordercolor="cccccc" bgcolor="#FFFFFF">
          <?
        if ($image['lx'] == 0) {
          echo  "会员下注";
      ...
      <tr>
           <td height="25" colspan="10" bordercolor="cccccc" bgcolor="#FFFFFF">
...
<select name="page2" onChange="location.href='main.php?action=list_look&uid=<?=$uid?>&vip=<?=$vip?>&kithe=<?=$kithe?>&class1=<?=$class1?>&class2=<?=$class2?>&class3=<?=$class3?>&lx=<?=$lx?>&page='+this.options[selectedIndex].value">
<?
$i = 1;
for (; $i <= $pagecount; ++$i) { 
    ?>
    <option value="<?=$i?>"
   <? if ($page == $i) { ?> selected<? }?>>第<?=$i?>页</option><? }?>
    </select>&nbsp; </td>
    </tr>
    </table>
    </td>
