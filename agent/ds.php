...
$sum_m = 0;
if ($zf_class2 == "特A" || $zf_class2 == "正A") {
    $result1 = $db1->query("select sum(if(username='" . $kauser . "',-sum_m," . $tb_zd . ")) as sum_sum from web_tan where kithe='" . $kithe . "' and class1='" . $zf_class1 . "' and class3='" . $zf_class3 . "' and " . $select_sql . "  and qx=0");
} else {
...
if ($sum_m < $zf_sum) {
    echo "<script>alert('对不起,(" . $zf_class1 . "-" . $zf_class2 . "-" . $zf_class3 . ")您" . $kithe . "期的占成金额：" . $sum_m . ",没有达到走飞金额：" . $zf_sum . "!');window.history.go(-1);</script>";
    exit;
}
$result = $db1->query("select ds_lb,rate,locked from web_bl where class1='" . $zf_class1 . "' and  class2='" . $zf_class2 . "' and class3='" . $zf_class3 . "' order by id LIMIT 1");
$row = $result->fetch_array();
if ($row['locked'] == 1 || $row == "" || $row['rate'] < 1) {
    echo "<script>alert('对不起,(" . $zf_class1 . "-" . $zf_class3 . ")已停押!');window.history.go(-1);</script>";
    exit;
}
$num = randstr();
$zf_dslb = $row['ds_lb'];
...
if ($pzall == 1 || $pzall == 2 || $pzall == 3) {
    if ($lx == 1 && $guan_temp['pz'] == 0 && $zong_temp['pz'] == 0 && $zong_ds_temp[$zf_dslb]['pz'] == 1 && 0 <= $zong_ds_temp[$zf_dslb]['pz_sum']) {
        if ($zf_class2 == "特A" || $zf_class2 == "正A") {
            $result1 = $db1->query("select sum(if(username='" . $zong . "',-sum_m,zongzc)) as sum_sum from web_tan where kithe='" . $kithe . "' and class1='" . $zf_class1 . "' and class3='" . $zf_class3 . "' and zong='" . $zong . "'  and qx=0");
        } else {
    ...
    if (($lx == 1 || $lx == 2) && $guan_temp['pz'] == 0 && $guan_ds_temp[$zf_dslb]['pz'] == 1 && 0 <= $guan_ds_temp[$zf_dslb]['pz_sum']) {
        if ($zf_class2 == "特A" || $zf_class2 == "正A") {
            $result1 = $db1->query("select sum(if(username='" . $guan . "',-sum_m,guanzc)) as sum_sum from web_tan where kithe='" . $kithe . "' and class1='" . $zf_class1 . "' and class3='" . $zf_class3 . "' and guan='" . $guan . "'  and qx=0");
        } else {
    ...
    if (($lx == 1 || $lx == 2 || $lx == 3) && $dagu_temp['zc'] == 0 && $dagu_ds_temp[$zf_dslb]['pz'] == 1 && 0 <= $dagu_ds_temp[$zf_dslb]['pz_sum']) {
        if ($zf_class2 == "特A" || $zf_class2 == "正A") {
            $result1 = $db1->query("select sum(if(username='" . $dagu . "',-sum_m,daguzc)) as sum_sum from web_tan where kithe='" . $kithe . "' and class1='" . $zf_class1 . "' and class3='" . $zf_class3 . "' and dagu='" . $dagu . "'  and qx=0");
        } else {
