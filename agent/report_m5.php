...
<body style="min-width: 1200px;width: 100%">
 ...
 &nbsp;&nbsp;类型:<? if ($class2 != "") {?><?=$class2?><?} else {?>全部<?}?>
           &nbsp;&nbsp;(以下所有结果均包含退水) &nbsp;&nbsp;代理:<?=$user?>--<a href="javascript:history.go( -1 );">回上一页</a> -- 
           <a href="main.php?action=report&uid=<?=$uid?>&vip=<?=$vip?>">回报表查询</a>
        </td>
        </tr>
        </tbody>
    </table></div></div>
    <? $result = $db1->query("SELECT username,sum(sz) as zs,sum(sum_m) as sum_m,sum(user_sf) usersf,sum(user_sq) user_ds,sum(dai_sq) dai_ds,sum(zong_sq) zong_ds,sum(guan_sq) guan_ds,sum(dagu_sq) dagu_ds,sum(daizc) as daizc,sum(dai_sf) as daisf,sum(zongzc) as zongzc,sum(zong_sf) as zongsf,sum(guanzc) as guanzc,sum(guan_sf) as guansf,sum(daguzc) as daguzc,sum(dagu_sf) as dagusf,sum(gszc) as gszc,sum(gs_sf) as gssf FROM " . $tb_name . "  WHERE visible=1 and dai='" . $user . "' and username!='" . $user . "' " . $vvv . " group by username order by username DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
    <tr> 
        <td height="18" colspan="11" align="center" nowrap="nowrap" class="t_list_caption_4">下级会员注单</td>
    </tr> 
    <tr > 
    ...
    <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
        ...
        <td align="center" nowrap="nowrap"><a href='main.php?action=report_m6&uid=<?=$uid?>&vip=<?=$vip?>&kithe=<?=$kithe?>&user=<?=$rs['username']?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>' ><?=round($sum_m, 2)?></a> </td>
        <td height="28" align="center" nowrap="nowrap"><?=round($usersf, 2)?></td>
        <td align="center" nowrap="nowrap"><?=round($userds, 2)?></td>
        <td align="center" nowrap="nowrap"><?=round($daizc, 2)?></td>
        <td align="center" nowrap="nowrap"><?=round($daids, 2)?></td>
        <td align="center" nowrap="nowrap"><?=round($daisf, 2)?></td>
        <td align="center" nowrap="nowrap"><?=get_sf(round(0 - $usersf, 2))?></td>
        <td align="center" nowrap="nowrap"><?=get_sf(round($usersf + $daisf, 2))?></td>
    </tr>
    <?}?>
    <tr class="t_list_tr_2">
        ...
        <td align="center" nowrap="nowrap"><?=round($z_daids1, 2)?></td>
        <td align="center" nowrap="nowrap"><?=round($z_daisf1, 2)?></td>
        <td align="center" nowrap="nowrap"><?=get_sf(round(0 - $z_usersf1, 2))?></td>
        <td align="center" nowrap="nowrap"><?=get_sf(round($z_usersf1 + $z_daisf1, 2))?></td>
    </tr>
    </table><br>
    <?}
     $result = $db1->query("SELECT username,sum(sz) as zs,sum(sum_m) as sum_m,sum(user_sf) usersf,sum(user_sq) user_ds,sum(zong_sq) zong_ds,sum(guan_sq) guan_ds,sum(dagu_sq) dagu_ds,sum(zongzc) as zongzc,sum(zong_sf) as zongsf,sum(guanzc) as guanzc,sum(guan_sf) as guansf,sum(daguzc) as daguzc,sum(dagu_sf) as dagusf,sum(gszc) as gszc,sum(gs_sf) as gssf FROM " . $tb_name . "  WHERE visible=1 and lx=1 and username='" . $user . "' " . $vvv . " group by username order by username DESC");
$ii = 0;
if ($result->num_rows != 0) {?>
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="t_list">
        <tr>
            <td height="18" colspan="7" nowrap="nowrap" class="t_list_caption_4">代理补货走飞结果</td>
        </tr>
        <tr>
            <td height="18" align="center" nowrap="nowrap"  class="t_list_tr_3">序号</td>
		...
		?>
          <tr bgcolor="#ffffff" onmouseover="javascript:this.bgColor='#FFFFA2'" onmouseout="javascript:this.bgColor='#ffffff'">
            ...
            <td align="center" nowrap="nowrap"><a href='main.php?action=report_m6&uid=<?=$uid?>?vip=<?=$vip?>&kithe=<?=$kithe?>&user=<?=$rs['username']?>&date_start=<?=$date_start?>&date_end=<?=$date_end?>&gtype=<?=$gtype?>&lx=1'><?=round($sum_m, 2)?></a></td>
            <td height="28" align="center" nowrap="nowrap">
                <?=round($usersf, 2)?>
            </td>
            <td align="center" nowrap="nowrap">
