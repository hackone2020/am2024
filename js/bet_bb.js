    ...
    switch (type) {
        ...
        case "blur":
            if (goldvalue != '') {
                ...
                if (rtype == 'SP' && (eval(goldvalue) > Number(bb_xxx))) {
                    gold.focus();
                    alert("对不起,此号下注金额最高限制:" + bb_xxx + "!!");
                    return false
                };
                if (rtype == 'SP' && (eval(goldvalue) > Number(bb_xx))) {
                    gold.focus();
                    alert("对不起,下注金额已超过单注限额:" + bb_xx + "!!");
                    return false
                };
                $("total_gold").value = Number($('allgold').innerHTML);
    ...
    if (http_request.readyState == 4) {
        if (http_request.status == 0 || http_request.status == 200) {
            ...
            for (var i = 0; i < 18; i++) {
                ...
                bl = "bl" + blid;
                if (blid >= 1 && blid <= 49) {
                    num2 = round(Number(num2) + Number(bb_blc), 3);
                    num3 = round(Number(num3) + Number(bb_blc), 3)
                };
                if (num6 == 1) {
                    $(bl).innerHTML = "<a>停</a>";
