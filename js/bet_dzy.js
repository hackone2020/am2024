    ...
    switch (type) {
        ...
        case "blur":
            if (goldvalue != '') {
                ...
                if (rtype == 'SP' && (eval(goldvalue) > Number(dzy_xxx))) {
                    gold.focus();
                    alert("对不起,此项下注金额最高限制:" + dzy_xxx + "!!");
                    return false
                };
                if (rtype == 'SP' && (eval(goldvalue) > Number(dzy_xx))) {
                    gold.focus();
                    alert("对不起,下注金额已超过单注限额:" + dzy_xx + "!!");
                    return false
                };
                $("total_gold").value = Number($('allgold').value);
    ...
    if (http_request.readyState == 4) {
        if (http_request.status == 0 || http_request.status == 200) {
            ...
            for (var i = 0; i < 49; i++) {
                ...
                bl = "bl" + blid;
                if (blid >= 1 && blid <= 49) {
                    num2 = round(Number(num2) + Number(dzy_blc), 3);
                    num3 = round(Number(num3) + Number(dzy_blc), 3)
                };
                if (num6 == 1) {
                    $(bl).innerHTML = "<a>停</a>";
