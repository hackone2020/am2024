    ...
    switch (type) {
        ...
        case "blur":
            if (goldvalue != '') {
                ...
                if (rtype == 'SP' && (eval(goldvalue) > Number(zm_xxx))) {
                    gold.focus();
                    alert("对不起,此号下注金额最高限制:" + zm_xxx + "!!");
                    return false
                };
                if (rtype == 'SP' && (eval(goldvalue) > Number(zm_xx))) {
                    gold.focus();
                    alert("对不起,下注金额已超过单注限额:" + zm_xx + "!!");
                    return false
                };
                if (rtype == 'ds' && (eval(goldvalue) > Number(zfds_xxx))) {
                    gold.focus();
                    alert("对不起,此号下注金额最高限制:" + zfds_xxx + "!!");
                    return false
                };
                if (rtype == 'ds' && (eval(goldvalue) > Number(zfds_xx))) {
                    gold.focus();
                    alert("对不起,下注金额已超过单注限额:" + zfds_xx + "!!");
                    return false
                };
                if (rtype == 'dx' && (eval(goldvalue) > Number(zfdx_xxx))) {
                    gold.focus();
                    alert("对不起,此号下注金额最高限制:" + zfdx_xxx + "!!");
                    return false
                };
                if (rtype == 'dx' && (eval(goldvalue) > Number(zfdx_xx))) {
                    gold.focus();
                    alert("对不起,下注金额已超过单注限额:" + zfdx_xx + "!!");
                    return false
                };
                $("total_gold").value = Number($('allgold').innerHTML);
    ...
    if (http_request.readyState == 4) {
        if (http_request.status == 0 || http_request.status == 200) {
            ...
            for (var i = 0; i < 53; i++) {
                ...
                bl = "bl" + blid;
                if (blid >= 1 && blid <= 49) {
                    num2 = round(Number(num2) + Number(zm_blc), 3);
                    num3 = round(Number(num3) + Number(zm_blc), 3)
                };
                if (blid >= 50 && blid <= 51) {
                    num2 = round(Number(num2) + Number(zfds_blc), 3);
                    num3 = round(Number(num3) + Number(zfds_blc), 3)
                };
                if (blid >= 52 && blid <= 53) {
                    num2 = round(Number(num2) + Number(zfdx_blc), 3);
                    num3 = round(Number(num3) + Number(zfdx_blc), 3)
                };
                if (num6 == 1) {
                    $(bl).innerHTML = "<a>停</a>";
